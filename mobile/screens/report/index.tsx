...
export default () => {
  ...
  const recordRef = React.useRef<BottomSheet>(null);
  const historyRef = React.useRef<BottomSheet>(null);
    ...
    <>
      ...
      >
        <View className="flex-1">
              ...
              >
                {fileReportItem.map((item: any, index: number) => (
                  <View
                    ...
                    className={`pr-4 ${index == 0 && "pl-6"} ${
                      index == fileReportItem.length - 1 && "pr-6"
                    }`}
                  >
          ...
          <View
            className="flex-1 flex-row items-center justify-center pt-6 px-6"
            style={{ gap: 60 }}
          >
            {user?.staff?.staff_id && (
              <DrawerTrigger bottomSheetRef={recordRef}>
            ...
            <DrawerTrigger bottomSheetRef={historyRef}>
              <View className="flex items-center justify-center gap-3">
                <View className="flex-row bg-white p-4 rounded-full shadow-lg elevation-sm">
      ...
      <DrawerView
        bottomSheetRef={recordRef}
        title={"Report Records"}
        description={"View all reports record"}
      >
        <ReportRecord />
      </DrawerView>
      <DrawerView
        bottomSheetRef={historyRef}
        title={"Report History"}
        description={"Your report submission history"}
      >
        <ReportHistory />
      </DrawerView>
    </>
