...
export default function LinkVerification() {
  ...
  const { control, trigger, getValues, setValue, clearErrors} = useRegistrationFormContext()
   const { mutateAsync: validateResident } = useValidateResident();
  const [inputMethod, setInputMethod] = useState<"id" | "manual" | null>(null)

  const handleSubmit = async () => {
    if (inputMethod === "id") {
      if (await trigger("linkAccountSchema.id")) {
        const rp_id = getValues('linkAccountSchema.id');
        validateResident({
          rp_id: rp_id
        }, {
          onSuccess: () => {
            router.push({
              pathname: "/registration/link/account-registration",
              params: {
                rp_id: rp_id
              }
            });
        ...
        <View className="px-5">
          ...
          <View className="mb-6 border-b border-gray-100 pb-4">
            ...
            <Text className="text-gray-600 text-sm font-PoppinsRegular text-center leading-6">
              Choose one of the options below to verify your existing resident profile.
            </Text>
          </View>
              ...
              >
                <View className="flex-row items-center justify-between">
                  <View className="flex-1">
                    <Text className="text-gray-900 text-base font-PoppinsSemiBold mb-1">I have my Resident ID</Text>
