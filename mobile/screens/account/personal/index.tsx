...
export default () => {
  ...
  const formatAddress = (address: any) => {
    ...
    if (address.sitio && address.sitio !== address.add_street)
      parts.push(`${address.sitio}`);
      ...
      <View className="flex-1 px-6">
        {user?.rp || !(user?.rp && user?.br) ? (
          <View className="flex-1">
            <InfoRow
              label="Full Name"
              value={`${personalData?.per_fname}${
                personalData?.per_mname ? ` ${personalData?.per_mname}` : ""
              } ${personalData?.per_lname} ${personalData?.per_suffix}`}
            />
            <InfoRow
              label="Sex & Date of Birth"
              value={`${personalData?.per_sex}  |  ${personalData?.per_dob}`}
            />
            <InfoRow label="Civil Status" value={personalData?.per_status} />
            <InfoRow
              label="Educational Attainment"
              value={personalData?.per_edAttainment}
            />
            ...
            {/* Address Section */}
            {personalData?.per_addresses &&
              personalData.per_addresses.length > 0 && (
                <View className="mt-6">
                  {personalData.per_addresses.map(
