...
interface ActiveMOMScreenProps {
  searchQuery: string
}

const INITIAL_PAGE_SIZE = 10;

export default function ActiveMOMScreen({ searchQuery }: ActiveMOMScreenProps) {
  ...
  const momRecords = activeMOMData?.results || []
  const totalCount = activeMOMData?.count || 0
    ...
    <View className="flex-1">
      ...
      {!isRefreshing && (
        <FlatList
          ...
          keyExtractor={(item) => `mom-active-${item.mom_id}`}
