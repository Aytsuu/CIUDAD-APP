...
import { Archive, ArchiveRestore, Trash, ChevronLeft } from 'lucide-react-native';
...
const CouncilCalendarPage = () => {
  ...
  useEffect(() => {
    console.log(`Events fetched for ${eventViewMode} (isArchived: ${isArchived}):`, events.map(e => ({
      ce_id: e.ce_id,
      ce_title: e.ce_title,
      ce_is_archive: e.ce_is_archive,
    })));
  }, [events, eventViewMode, isArchived]);
  ...
  const formattedEvents = events
    .filter(event => {
      ...
      if (!matchesArchiveStatus) {
        console.warn(`Event ce_id: ${event.ce_id} has ce_is_archive: ${event.ce_is_archive} but expected ${isArchived}`);
      }
        ...
        <View className="flex-row justify-end space-x-2 mt-2">
          {item.is_archive ? (
            ...
            <ConfirmationModal
              ...
              actionLabel="Archive"
              onPress={() => deleteEventMutation.mutate({ ce_id: item.ce_id, permanent: false })}
          ...
          {eventViewMode === 'archive' && (
            <ConfirmationModal
              ...
              actionLabel="Delete"
              variant="destructive"
              onPress={() => deleteEventMutation.mutate({ ce_id: item.ce_id, permanent: true })}
              loading={deleteEventMutation.isPending}
            />
          )}
        </View>
  ...
  if (error) {
    return (
      <SafeAreaView className="flex-1 bg-gray-50 justify-center items-center">
        <Text className="text-red-500 text-lg">Failed to load events: {error.message}</Text>
      </SafeAreaView>
    );
  }
    ...
    >
      ...
      <View className="flex-row justify-center my-3">
        <View className="flex-row border border-gray-300 rounded-full bg-gray-100 overflow-hidden">
          <TouchableOpacity
            className={`px-4 py-2 ${eventViewMode === 'active' ? 'bg-white' : ''}`}
            onPress={() => {
              setEventViewMode('active');
              queryClient.invalidateQueries({ queryKey: ['councilEvents', false] });
            }}
          >
            <Text className="text-sm font-medium">Active</Text>
          </TouchableOpacity>
          <TouchableOpacity
            className={`px-4 py-2 ${eventViewMode === 'archive' ? 'bg-white' : ''}`}
            onPress={() => {
              setEventViewMode('archive');
              queryClient.invalidateQueries({ queryKey: ['councilEvents', true] });
            }}
          >
      ...
      <View className="flex-1 px-4 pt-4">
        ...
        {filteredEvents.length > 0 ? (
          ...
          <View className="flex-1 justify-center items-center">
            ...
            <Text className="text-gray-400 mt-4 text-lg">No events scheduled</Text>
          </View>
        )}
      </View>
      </ScrollView>
