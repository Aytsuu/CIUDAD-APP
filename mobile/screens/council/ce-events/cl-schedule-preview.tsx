...
const CLSchedulePreview = () => {
  ...
  const event = useMemo(() => (params.event ? JSON.parse(params.event as string) : null), [params.event]);
  ...
  const { data: attendees = [], isLoading: isAttendeesLoading, refetch } = useGetAttendees(ceId);
  const { control, handleSubmit, reset, watch, getValues } = useForm<EventFormValues>({
    ...
    const options = staffList.map((staff: Staff, index: number) => {
      const attendee = attendees.find(
        (a: any) =>
          ...
          normalizeString(a.atn_designation) === normalizeString(staff.position_title || '')
      );
      return {
        id: attendee ? String(attendee.atn_id) : String(staff.staff_id || index + 1),
        name: `${staff.full_name} (${staff.position_title || 'No Designation'})`,
      };
    });
    console.log('Raw staffList:', staffList);
    console.log('staffOptions:', options.map(o => ({ id: o.id, name: o.name })));
    return options;
  }, [staffList, attendees]);

  const selectedAttendeeDetails = useMemo(() => {
    const filteredAttendees = attendees.filter((attendee: any) => String(attendee.ce_id) === String(ceId));
    return filteredAttendees.map((attendee: any) => ({
      ce_title: attendee.ce_title || event?.ce_title || event?.title || 'Unknown',
  ...
  const getStaffById = (id: string): Staff | undefined => {
    ...
    const attendee = attendees.find((a: any) => String(a.atn_id) === normalizedId);
    if (attendee) {
  ...
  }, [attendees, isAttendeesLoading, ceId, reset, selectedAttendeeDetails, isAdding, getValues]);

  useEffect(() => {
    if (isEditMode && eventCategory === 'meeting' && staffAttendees.length && ceId) {
      const newAttendees = staffAttendees
        .map((id: string) => {
          ...
          const existingAttendee = selectedAttendeeDetails.find((a) => a.atn_id === Number(id));
          return {
  ...
  }, [staffAttendees, selectedAttendeeDetails, isEditMode, eventCategory, ceId, event, staffList]);
  ...
  const haveAttendeesChanged = () => {
    const hasChanged = selectedAttendees.length !== initialAttendees.length ||
    ...
    console.log('haveAttendeesChanged:', hasChanged, 'selectedAttendees:', selectedAttendees, 'initialAttendees:', initialAttendees);
    return hasChanged;
  };
        ...
        },
        {
          onSuccess: () => {
            if (data.eventCategory === 'meeting' && selectedAttendees.length && haveAttendeesChanged()) {
              ...
              updateAttendees(attendeePayload, {
                onSuccess: () => refetch().catch((err) => console.error('Refetch error:', err.message)),
              });
            } else {
              console.log('No attendee update triggered: eventCategory:', data.eventCategory, 'selectedAttendees.length:', selectedAttendees.length, 'haveAttendeesChanged:', haveAttendeesChanged());
              refetch().catch((err) => console.error('Refetch error:', err.message));
            }
            setIsEditMode(false);
          },
        },
              ...
              <>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    reset({
                      ...getValues(),
                      staffAttendees: initialAttendees.map(a => String(a.atn_id)),
                    });
                  }}
                >
                  <Text>Cancel</Text>
