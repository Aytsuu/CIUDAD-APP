...
const ResidentDonationMain = () => {
  ...
  useEffect(() => {
    const handleDeepLink = (url: string | null) => {
      if (!url) return;
      if (
        url.includes("checkout.paymongo.com/mobile/return") &&
        currentPaymentId
      ) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { checkout_url, payment_intent_id } = await createDonationPayment(
        amountNum
      );
      ...
      try {
        ...
        if (status.paid) {
          ...
          setErrorModal({
            ...
            message:
              "The payment attempt was not successful. Please try again.",
          });
        }
          ...
          <View className="flex-1 justify-center items-center bg-black/50">
            <View className="bg-white p-6 rounded-lg w-80">
              ...
              <Text className="text-xs text-center text-gray-500 mb-4">
                "Every peso is accounted for and directly benefits Barangay San
                Roque residents."
              </Text>
