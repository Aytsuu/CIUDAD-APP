...
import { LoadingState } from "@/components/ui/loading-state"; // ✅ Import LoadingState

export default function ResidentAccepted() {
  const [searchQuery, setSearchQuery] = useState("");
  const { user } = useAuth();

  const { data: acceptedRequest = [], isLoading: isDataLoading } = useGetGarbageAcceptedResident(String(user?.rp));
    ...
    <View className="flex-1 p-6">
      ...
      {isDataLoading ? (
        <View className="h-64 justify-center items-center">
          <LoadingState />  {/* ✅ Replaced text loading with animated LoadingState */}
        </View>
