...
//     const amount = tracking.payment_request?.pay_status; // Assuming this is the field for amount
//     const dueDate = tracking.payment_request?.pay_due_date;
...
//                     : (dueDate ? formatDate(dueDate, "long") : "N/A")
//                   }
//                 </Text>
//               </View>
//             </View>
...
//       const caseStatus = tracking.summon_case?.sc_conciliation_status ? tracking.summon_case.sc_conciliation_status : tracking.summon_case?.sc_mediation_status
//       steps.push({
//         id: 2,
...
export default function CaseTrackingScreen({
  ...
  if(isRaised == "Raised"){
    ...
    const caseStatusDisplay = getCaseStatusDisplay();
   ...
   const getSteps = () => {
    ...
    const dueDate = tracking.payment_request?.pay_due_date;
      ...
      steps.push({
        ...
        title: "Schedule Hearing",
        description: "Attend the scheduled mediation session with both parties.",
        status: caseStatusDisplay.toLowerCase(),
        display_status: caseStatusDisplay,
        details: tracking.hearing_schedules?.length
          ? `${tracking.hearing_schedules?.length} hearing session(s) scheduled`
          : "Waiting for payment confirmation to schedule hearing.",
      });
      ...
      steps.push({
        ...
        display_status: caseDisplay,
        details: tracking.summon_case?.sc_date_marked
          ? `Case ${caseStatusDisplay} on ${formatDate(tracking.summon_case.sc_date_marked)}`
          : "Pending mediation outcome.",
      });
                  ...
                  <View className="flex-row justify-between items-center">
                    <View className="flex-row items-center space-x-2 gap-2">
                      <CheckCircle2
                        size={16}
                        className={
                          caseStatusDisplay === "Resolved"
                            ? "text-emerald-500"
                            : caseStatusDisplay === "Ongoing" 
                            ? "text-blue-500"
                            : caseStatusDisplay === "Waiting for Schedule" 
                            ? "text-amber-500"
                            : "text-red-500"
                        }
                      />
                    ...
                    {getStatusBadge(caseStatusDisplay)}
                  </View>
                </View>
              </Card>
            </View>
