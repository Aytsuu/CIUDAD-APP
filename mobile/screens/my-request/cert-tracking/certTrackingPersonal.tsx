...
export default function CertTrackingPersonal() {
  ...
  try { console.log('[CertTrackingPersonal] rp used:', rp); } catch {}
  ...
  const getUiStatus = (item: any): 'pending' | 'approved' | 'completed' => {
    ...
    const hasIssuedDate = Boolean(item?.cr_date_completed || item?.date_completed || item?.ic_date_of_issuance);
      ...
      <View className="flex-1 p-6">
        ...
        {isLoading ? (
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {data?.personal?.filter((item: any) => (item?.cr_req_status ?? '').toString().toLowerCase() === activeTab || getUiStatus(item) === activeTab).length ? (
              data.personal
                .filter((item: any) => (item?.cr_req_status ?? '').toString().toLowerCase() === activeTab || getUiStatus(item) === activeTab)
