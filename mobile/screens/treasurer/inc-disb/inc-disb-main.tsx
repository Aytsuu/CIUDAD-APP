...
const IncomeandDisbursementMain = () => {
  ...
  const handleAction = useCallback((action: string, items: ImageItem | ImageItem[] | Album, onSuccess?: () => void) => {
    ...
    // Handle album deletion by deleting all images
    if (action === "permanentDelete" && "id" in items && !("infi_num" in items || "disf_num" in items)) {
      const album = items as Album;
      const mutations = album.images.map((item) => {
            ...
            <>
              ...
              <View>
                <ConfirmationModal
                  ...
                  actionLabel="Delete"
                  variant="destructive"
                  onPress={() => handleAction("permanentDelete", album.images)} // Use album.images to match web version
                />
              </View>
            </>
          )}
