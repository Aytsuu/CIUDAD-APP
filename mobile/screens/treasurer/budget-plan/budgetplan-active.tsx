...
import { usegetBudgetPlanActive } from "./queries/budgetPlanFetchQueries";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import { BudgetPlanType } from "./queries/budgetPlanFetchQueries";
import { ConfirmationModal } from "@/components/ui/confirmationModal";
import { useArchiveBudgetPlan } from "./queries/budgetPlanUpdateQueries";
import { useRouter } from "expo-router";
import { LoadingState } from "@/components/ui/loading-state";

interface ActiveBudgetPlanScreenProps {
  searchQuery: string;
}

const INITIAL_PAGE_SIZE = 10;

export default function ActiveBudgetPlanScreen({ searchQuery }: ActiveBudgetPlanScreenProps) {
  ...
  const { 
    ...
    refetch,
    isFetching
  } = usegetBudgetPlanActive(currentPage, pageSize, searchQuery);

  const plans = activePlansData?.results || [];
  const totalCount = activePlansData?.count || 0;
  const hasNext = activePlansData?.next;
    ...
    <View className="flex-1">
      ...
      {!isRefreshing && (
        <FlatList
          ...
          keyExtractor={(item) => `budget-active-${item.plan_id}`}
