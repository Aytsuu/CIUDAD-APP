...
const getValidResidentProfileId = async (userId?: string) => {
    if (!userId) {
        // Don't throw here, let the caller handle validation if needed
        return null; 
    }
    return userId;
};
...
export const addCertificationRequest = async (requestInfo: Record<string, any>, staffId?: string, userId?: string) => {
    try {
        ...
        if (requestInfo.cert_type === 'personal') {
            ...
            let residentProfileId = requestInfo.rp_id || userId;
        ...
        // Permit Certification (BusinessPermitRequest model)
            ...
            const rpId = requestInfo.rp_id || userId;
            const brId = requestInfo.br_id;
            
            if (!rpId && !brId) {
                throw new Error("Error: No user ID provided (rp_id or br_id required)");
            }
