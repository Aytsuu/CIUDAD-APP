...
const AdminProjectProposalView: React.FC<{
    ...
    <ScrollView className="flex-1 bg-white">
      ...
      <View className="p-4">
        ...
        <View className="mb-4">
          ...
          {!objectives ||
          objectives.length === 0 ||
          objectives.every((obj) => !obj || !obj.trim()) ? (
        ...
        <View className="mb-4">
          ...
          budgetItems.every((item) => !item.name || !item.name.trim()) ? (
  ...
  const renderSupportingDocument = () => {
    const nonArchivedDocs = supportDocs.filter((doc) => !doc.psd_is_archive);

    return (
      <ScrollView className="flex-1 bg-white p-4">
        {nonArchivedDocs.length > 0 ? (
          ...
          <View className="flex-1 justify-center items-center">
            <Text className="text-gray-500 text-center">
              {supportDocs.length > 0
                ? "No non-archived supporting documents available"
      ...
      >
        ...
        >
          <View className="absolute top-32 left-4 right-4 bg-white rounded-lg shadow-lg border border-gray-200">
            {["All", "Approved", "Pending", "Rejected", "Viewed"].map(
              (filter) => (
                <TouchableOpacity
                  key={filter}
                  className={`px-4 py-3 border-b border-gray-100 ${
                    filter === selectedFilter ? "bg-blue-50" : ""
                  }`}
                  onPress={() => {
