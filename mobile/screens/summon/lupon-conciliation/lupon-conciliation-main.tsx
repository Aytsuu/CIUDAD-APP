...
export default function LuponConciliationMain() {
    ...
    const { 
        ...
        refetch,
        isFetching 
    } = useGetLuponCaseList(currentPage, pageSize, searchQuery, selectedFilter);
    ...
    return (
        <PageLayout
            ...
            headerTitle={
                <Text className="text-gray-900 text-[13px]">Conciliation Proceedings</Text>
            }
            rightAction={
                <TouchableOpacity
        ...
        >
            ...
            <View className="flex-1 px-6">
                ...
                {!isRefreshing && summonCases.length > 0 && (
                    <Text className="text-xs text-gray-500 mt-2 mb-3">{`Showing ${summonCases.length} of ${totalCount} conciliation cases`}</Text>
                )}
                ...
                {!isRefreshing && (
                    <FlatList
                        ...
                        keyExtractor={(item) => `lupon-${item.sc_id}`}
