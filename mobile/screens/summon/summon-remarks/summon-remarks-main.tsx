...
export default function SummonRemarksMain() {
    ...
    const { 
        ...
        refetch,
        isFetching 
    } = useGetSummonCaseList(currentPage, pageSize, searchQuery, selectedFilter);
    ...
    // ================= SIDE EFFECTS =================
    // EXACT SAME as ResidentRecords - handles clearing search
    React.useEffect(() => {
    ...
    // Simple empty state component - FIXED for center center
    ...
    return (
        <PageLayout
            ...
            headerTitle={
                <Text className="text-gray-900 text-[13px]">Summon Remarks</Text>
            }
            rightAction={
                <TouchableOpacity
            ...
            wrapScroll={false}
        >
            {/* Search Bar - EXACT SAME as ResidentRecords */}
            {showSearch && (
                <SearchInput
            ...
            <View className="flex-1 px-6">
                ...
                {/* Result Count - EXACT same as ResidentRecords */}
                {!isRefreshing && summonCases.length > 0 && (
                    <Text className="text-xs text-gray-500 mt-2 mb-3">{`Showing ${summonCases.length} of ${totalCount} summon cases`}</Text>
                )}
                
                {/* Loading state during refresh - EXACT same as ResidentRecords */}
                {isFetching && isRefreshing && !isLoadMore && <LoadingState />}

                {/* Main Content - only render when not refreshing - EXACT same as ResidentRecords */}
                {!isRefreshing && (
                    <FlatList
                        ...
                        keyExtractor={(item) => `summon-${item.sc_id}`}
                        removeClippedSubviews
                        contentContainerStyle={{
                            paddingTop: 0,
                            paddingBottom: 20,
                            flexGrow: 1, // THIS IS THE KEY - makes empty state center properly
                        }}
                        refreshControl={
                            <RefreshControl
