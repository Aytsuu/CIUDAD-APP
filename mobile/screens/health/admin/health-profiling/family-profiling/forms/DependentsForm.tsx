...
export const DependentsStep: React.FC<DependentsStepProps> = ({ form, onFamilyCreated, staffId, onBack }) => {
  ...
  // Map building type to expected backend values
  const mapBuildingType = (building: string): string => {
    const buildingMap: Record<string, string> = {
      'owner': 'Owner',
      'renter': 'Renter',
      'other': 'Sharer', // Backend expects 'Sharer' for 'other'
    };
    return buildingMap[building.toLowerCase()] || 'Owner';
  };
  ...
  const handleCreateFamilyInternal = async () => {
    try {
      ...
      //   fam_building: mapBuildingType(formData.demographicInfo.building),
      ...
      // Step 2: Create Family Composition (matching web - uses "Mother", "Father", "Guardian")
      ...
      // Add Guardian if selected
      if (formData.guardInfo?.id) {
        parentCompositions.push({
          fam: famId,
          rp: formData.guardInfo.id.split(' ')[0],
          fc_role: 'Guardian',
        });
      }
