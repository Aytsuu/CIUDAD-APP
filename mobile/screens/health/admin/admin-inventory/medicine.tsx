  ...
  const fetchInventoryData = React.useCallback(async () => {
    ...
    try {
      ...
      if (failedEndpoints.length > 0) {
        setHasError(true)
        setErrorMessage(`Partial data loaded. Failed to fetch: ${failedEndpoints.map(f => f.name).join(', ')}`)
      }
                ...
                return (
                  <View className="bg-white rounded-xl shadow-sm mb-3 p-4">
                    <View className="flex-row justify-between items-start mb-2">
                      ...
                      <View className={`px-2 py-1 rounded-full ${stockStatus.status === 'out_of_stock' ? 'bg-red-50' : stockStatus.status === 'low_stock' ? 'bg-orange-50' : 'bg-green-50'}`}>
                        <Text className={`text-xs font-medium ${stockStatus.status === 'out_of_stock' ? 'text-red-600' : stockStatus.status === 'low_stock' ? 'text-orange-600' : 'text-green-600'}`}>
                          {stockStatus.text}
                        </Text>
                      </View>
                    </View>
