...
const WasteEventSchedSchema = z.object({
    ...
    date: z.string().nonempty("Date is required"), // Ensures the date is a non-empty string
    time: z.string().nonempty("Time is required"), // Ensures the time is a non-empty string
    organizer: z.string().nonempty("Organizer is required"), // Ensures the organizer is a non-empty string
    invitees: z.string().optional(), // Optional field for invitees
