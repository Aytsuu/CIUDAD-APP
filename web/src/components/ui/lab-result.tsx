...
import { MediaUpload, MediaUploadType } from "@/components/ui/media-upload"; // Adjust path as needed
              ...
              <MediaUpload
                ...
                setMediaFiles={(mediaFiles) => {
                  if (typeof mediaFiles === "function") {
                    // If MediaUpload ever passes a function, call it with current value
                    onLabMediaFiles(
                      labName,
                      mediaFiles(labData.mediaFiles)
                    );
        ...
        {labData.toBeFollowed && (
          <div className="text-sm text-orange-600 bg-orange-50 p-3 rounded-md border border-orange-200">
            ...
            <p className="mt-1 text-orange-700">
              This laboratory result will be followed up later.
            </p>
          </div>
        )}
      </div>
...
export default function LaboratoryResults({ labResults, onLabResultsChange }: LaboratoryResultsProps) {
  ...
  const handleLabCheckbox = (labName: string, checked: boolean) => {
    const updatedResults = {
      ...
      [labName]: {
        ...
        ...(checked ? {} : { date: "", mediaFiles: [], toBeFollowed: false })
      }
    };
    onLabResultsChange(updatedResults);
  };

  const handleLabDate = (labName: string, date: string) => {
    const updatedResults = {
      ...labResults,
  ...
  const handleLabMediaFiles = (labName: string, mediaFiles: MediaUploadType) => {
    const updatedResults = {
      ...labResults,
      [labName]: { ...labResults[labName], mediaFiles }
    };
    onLabResultsChange(updatedResults);
  };

  const handleToBeFollowed = (labName: string, toBeFollowed: boolean) => {
    const updatedResults = {
      ...labResults,
      [labName]: { 
        ...labResults[labName], 
                  ...
                  <div className="flex-1 text-right">
                    {lab.toBeFollowed ? (
                      ...
                      <div className="text-sm text-gray-600">
                        ...
                        {!lab.date && lab.mediaFiles.length === 0 && (
