...
export default function ComplaintSectionCharts() {
  const { showLoading, hideLoading } = useLoading();
  const { data: complaintChartAnalytics = [], isLoading, error } = useGetComplaintChartAnalytics();
      ...
      <Card className="w-full border-none shadow-none">
        <CardContent className="flex items-center justify-center py-8">
          <Alert variant="destructive" className="max-w-md">
            ...
            <AlertDescription>Failed to load complaint chart data. Please try again later.</AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="w-full border-none shadow-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x bg-gray-50/50 border-b border-gray-100">
        ...
        <div className="px-6 py-4">
          ...
          <div className="text-2xl font-bold text-gray-900">
            {averageComplaints.toLocaleString()}
          </div>
        </div>
      ...
      <CardContent className="p-6">
        {complaintChartAnalytics.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-gray-600 max-w-sm">
              There are no complaint cases to display for the selected time period.
            </p>
          </div>
        ) : (
