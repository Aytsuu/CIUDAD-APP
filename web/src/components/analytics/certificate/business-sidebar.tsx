import { useGetBusinessPermitSidebar } from "./certificate-analytics-queries";
import { Card } from "@/components/ui/card";
import { ChevronRight, CheckCircle, XCircle, AlertCircle, Building2, Clock } from "lucide-react";
...
export const BusinessSidebar = () => {
  ...
  const recentRequests = businessData?.recent_requests || [];
        ...
        {isLoading ? (
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-medium text-gray-700 truncate mb-1">
                        {request.bus_permit_name || 'Business Permit'}
                      </h3>
                      
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xs text-gray-500">
                          {formatTimeAgo(request.req_request_date)}
                        </span>
                        <div className={`flex items-center gap-1 text-xs font-semibold ${getStatusColor(request.req_status)}`}>
                          {getStatusIcon(request.req_status)}
                          {request.req_status}
                        </div>
                      </div>
          ...
          <div className="flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-sm text-gray-500">
              Business permit requests recently submitted will appear here
            </p>
          </div>
        )}
      </div>
