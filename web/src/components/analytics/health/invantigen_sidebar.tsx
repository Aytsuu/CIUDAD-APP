...
export function AntigenAlertsSidebar() {
  const [currentPage] = useState(1);
  const [pageSize] = useState(20);
  const { data: apiResponse, isLoading } = useAntigenCombineStocks(currentPage, pageSize, "", "all");

  // Transform antigen data into alert items - UPDATED to handle combined statuses
  const getAlertItems = (): StockAlertItem[] => {
    ...
    apiResponse.results.forEach((item: any) => {
      // Only include items that have at least one alert status
  ...
  // Dynamic function to get all status badges for an item
        ...
        {!hasAlerts ? (
          ...
          <>
              ...
              {itemsToShow.map((item) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    {/* Dynamic badge container - shows multiple badges if needed */}
                  ...
                  <div className="text-xs text-muted-foreground space-y-1">
                    ...
                    {item.item?.dosage && (
                      <p className="text-xs opacity-75">
                        {item.item.dosage} {item.item.unit || ""}
                        {item.type && ` • ${item.type === 'vaccine' ? 'Vaccine' : 'Supply'}`}
                      </p>
                    )}
                    
                    {!item.item?.dosage && item.type && (
                      <p className="text-xs opacity-75">
                        {item.type === 'vaccine' ? 'Vaccine' : 'Immunization Supply'}
                      </p>
                    )}
                  </div>
                </div>
            ...
            <div className="pt-3 border-t border-gray-100 mt-3">
              <Link to="/inventory-stocks/list/stocks/antigen">
                <Button 
                  variant="link" 
