import {
  ...
  export function MedicineDistributionChart({ initialMonth }: MedicineChartProps) {
    ...
    const topN = 10;
    const displayData = allMedicines.slice(0, topN);
    const othersCount = allMedicines.slice(topN).reduce((sum, item) => sum + item.count, 0);
    
    if (othersCount > 0) {
      displayData.push({ name: "Other Medicines", count: othersCount });
    }
      ...
      if (active && payload && payload.length) {
        return (
          <div className="bg-white p-4 shadow-md rounded-md border border-gray-200">
            ...
            {payload[0].payload.name === "Other Medicines" && otherMedicinesCount > 0 && (
              <p className="text-sm mt-1">
                (Combines {otherMedicinesCount} other medicines)
              </p>
            )}
          </div>
        );
          ...
          content={
            <Alert variant="destructive" className="m-4">
              ...
              <AlertDescription className="text-sm">
                Failed to load medicine distribution data. Please try again later.
              </AlertDescription>
            </Alert>
          }
    ...
    return (
      <CardLayout
        ...
        description={`Most requested medicne in ${format(
          currentDate,
          "MMMM yyyy"
        )}`}
        content={
              ...
              <>
                <div className="text-sm text-gray-500 text-center mb-2">
                  ...
                  {otherMedicinesCount > 0 && (
                    <span> and {otherMedicinesCount} others combined</span>
                  )}
                </div>
