          ...
          <div className="p-4 space-y-3">
            {incomes.slice(0, 2).map((income) => (
              <DialogLayout
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-col gap-1 text-xs text-gray-500">
                          ...
                          <span>{new Date(income.gbud_datetime).toLocaleDateString()}</span>
                        </div>
                      </div>
                      <ChevronRight className="w-4 h-4 text-gray-400 mt-1" />
                    </div>
                ...
                description={`Entry #${income.gbud_num}`}
                mainContent={
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                      ...
                      <div>
                        <Label>Amount</Label>
                        <p>₱{income.gbud_amount?.toLocaleString()}</p>
                      </div>
                      <div>
                        <Label>Date</Label>
                        <p>{new Date(income.gbud_datetime).toLocaleDateString()}</p>
                      </div>
                    </div>
                    {income.gbud_add_notes && (
                      <div>
                        <Label>Description</Label>
                        <p className="whitespace-pre-wrap">{income.gbud_add_notes}</p>
                      </div>
                    )}
                  </div>
                }
