...
import { ChevronLeft, Check, CircleAlert, Forward, Calendar } from "lucide-react"
    ...
    },
    {
        ...
        cell: ({ row }) => {
          ...
          if (remark && remark.rem_id) {
            ...
            return (
              <div className="text-red-500 flex items-center gap-1 justify-center text-center">
                  <CircleAlert size={16} />
    ...
    <div className="w-full h-full">
      <div className="overflow-y-auto h-full">
        ...
        <Card className="w-full bg-white p-6 shadow-sm mb-8">
          {/* 3rd Mediation Action Notice - Inside the card */}
          {isThirdMediation && !isCaseClosed && (
            <Alert className="bg-red-50 border-red-200 mb-6">
              <div className="flex items-center gap-2">
                <InfoIcon className="h-4 w-4 text-red-600" />
                <AlertDescription className="text-red-800">
                  <strong>3rd Mediation Reached:</strong> This case has reached the final mediation level. You can either mark the case as resolved or forward it to Lupon Tagapamayapa for further action.
                </AlertDescription>
              </div>
            </Alert>
          )}
              ...
              <div className="flex gap-3">
                ...
                {isThirdMediation && (
                  <TooltipLayout
                    content={
                      !allSchedulesHaveRemarks && !allSchedulesAreClosed 
                        ? "All hearing schedules must have remarks and be closed before forwarding" 
                        : !allSchedulesHaveRemarks 
                        ? "All hearing schedules must have remarks before forwarding"
                        : !allSchedulesAreClosed
                        ? "All hearing schedules must be closed before forwarding"
                        : "Forward case to Lupon Tagapamayapa"
                    }
                    trigger={
                      <div>
                        <ConfirmationModal
