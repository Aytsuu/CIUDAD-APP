...
export function getInitialFormData(
  ...
  const forms = cloneInitialForms();

  if (isEdit && details?.length) {
    for (const detail of details) {
      if (!isBudgetItem(detail.dtl_budget_item)) continue;
      const fieldName = fieldMapping[detail.dtl_budget_item];
      assignToForm(forms, fieldName, detail.dtl_proposed_budget.toString());
    }
  }
