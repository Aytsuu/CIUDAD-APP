...
//   // Helper function to get available transfer amount from expense particulars
//   const getAvailableTransferAmount = (dtl_id: string) => {
...
//   // Source items (from) - exclude items with 0 available amount (including unappropriated if 0)
//   const fromOptions = [
...
//     ...budgetItems
//       .filter(item => {
//         const availableAmount = getAvailableTransferAmount(item.dtl_id?.toString() || "");
...
//           transferrableAmount: availableAmount,
//         }
//       })
//   ];
...
//       const fromAmount = fromValue ? Number(getAvailableTransferAmount(fromValue)) : 0;
...
//         const availableAmount = getAvailableTransferAmount(fromValue);
//         const enteredAmount = Number.parseFloat(amountValue) || 0;

//         if (enteredAmount > availableAmount) {
//           form.setError(`items.${index}.amount`, {
//             type: "manual",
//             message: `Amount cannot exceed available ₱${availableAmount.toFixed(2)}`,
//           });
//         } else {
...
//   // Helper to display available amount with tooltip
//   const getAvailableAmountDisplay = (index: number) => {
//     const fromValue = formValues.items?.[index]?.from;
//     if (!fromValue) return { display: "0.00", original: "0.00", isUsingExpenseData: false };
...
//       isUsingExpenseData: fromValue !== "unappropriated"
//     };
//   };
...
//             const amountValue = formValues.items?.[index]?.amount;
//             const fromValue = formValues.items?.[index]?.from;
//             const toAmount = toValue ? Number(getAvailableTransferAmount(toValue)) : 0;
//             const enteredAmount = Number.parseFloat(amountValue) || 0;
...
//             const isOverTransferLimit = Number(availableInfo.display) < 0;
//             const transferrableAmount = fromValue ? getAvailableTransferAmount(fromValue) : 0;

//             return (
//               <div
//                 key={field.id}
//                 className={`
//                   grid grid-cols-[2fr_1fr_1fr_2fr_1.5fr_1fr_0.5fr] gap-3 items-start p-2 rounded-lg mb-2
