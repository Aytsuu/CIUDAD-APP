...
import { useDeleteDisbursementFile } from "./queries/incDisb-delqueries";
...
export const EditDisbursementVoucher: React.FC<DisbursementVoucherFormProps> = ({
  onSuccess,
  existingVoucher,
}) => {
  const { user } = useAuth();
  const [files, setFiles] = useState<MediaUploadType>([]);
  const [existingFiles, setExistingFiles] = useState<DisbursementFile[]>([]);
  const [activeVideoId, setActiveVideoId] = useState<string>("");
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const updateMutation = useUpdateDisbursementVoucher();
  const addFilesMutation = useAddDisbursementFiles();
  const deleteFilesMutation = useDeleteDisbursementFile();
  ...
  useEffect(() => {
    if (existingVoucher) {
      ...
      // Set existing files separately
      setExistingFiles(existingVoucher.files || []);

      // Set files for MediaUpload component (only new files will have file property)
      setFiles(
        (existingVoucher.files || []).map((file: DisbursementFile) => ({
  ...
  const onSubmit = async (data: DisbursementFormValues) => {
    try {
      ...
      if (!existingVoucher?.dis_num) {
        throw new Error('No disbursement voucher ID provided for update.');
      }
      ...
      const disbursementData: DisbursementInput = {
        dis_num: existingVoucher.dis_num,
      ...
      // Handle files like the project proposal does
      const currentFiles = files;
      const currentFileUrls = currentFiles.map(file => file.url).filter(Boolean);
      
      // Identify files to delete (files that were in existingFiles but not in currentFiles)
      const filesToDelete = existingFiles.filter(existingFile => 
        !currentFileUrls.includes(existingFile.disf_url)
      );
      
      // Identify new files to add (files that have a file property but no URL)
      const newFiles: FileInput[] = currentFiles
      ...
      if (filesToDelete.length > 0) {
        for (const file of filesToDelete) {
          await deleteFilesMutation.mutateAsync({
            disNum: existingVoucher.dis_num,
            disfNum: file.disf_num,
          });
        }
      }
      ...
      let errorMessage = "Failed to update disbursement voucher. Please check the form data and try again.";
