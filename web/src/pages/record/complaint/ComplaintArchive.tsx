...
const ArchiveComplaints = () => {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      const formattedComplaints = response.data.map((complaint: any) => ({
        ...
        dateArchived: complaint.comp_updated_at || new Date().toISOString(),
        status: "Archived",
        priority: complaint.comp_category,
        accusedPersons:
          complaint.accused_persons?.map((a: any) => a.acsd_name) || [],
  ...
  useEffect(() => {
    filterComplaints();
  }, [searchTerm, selectedDateRange, complaints]);
  ...
  const filterComplaints = () => {
    ...
    if (selectedDateRange !== "all") {
      ...
      if (cutoffDate) {
        filtered = filtered.filter(
          (complaint) => new Date(complaint.dateArchived) >= cutoffDate
        );
      }
    }
    ...
    try {
      ...
      if (failureCount === 0) {
        toast.success("Bulk restore successful", {
          description: `${successCount} complaint(s) have been restored.`,
        });
      } else {
        toast.warning("Partial restore completed", {
          description: `${successCount} restored, ${failureCount} failed.`,
        });
      }
    } catch (error: any) {
      console.error("Failed to bulk restore:", error);
      toast.error("Failed to restore complaints", {
        description: error.response?.data?.message || error.message,
      });
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to permanently delete ${selectedComplaints.length} complaint(s)? This action cannot be undone.`
      )
    )
      return;
    ...
    try {
      ...
      if (failureCount === 0) {
        toast.success("Bulk delete successful", {
          description: `${successCount} complaint(s) have been deleted.`,
        });
      } else {
        toast.warning("Partial delete completed", {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="flex gap-3 mb-2">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-darkBlue2">
                Archived Complaints
              </h1>
      ...
      <div className="mt-6 flex justify-between items-center">
        <div className="text-sm text-gray-600">
          Showing {filteredComplaints.length} of {complaints.length} archived
          complaints
        </div>
        <button
