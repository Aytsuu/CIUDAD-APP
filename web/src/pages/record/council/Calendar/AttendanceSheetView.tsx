...
// const sanRoqueLogoBase64 = sanroque_logo;
...
//       body: chunk.length > 0
//         ? chunk.map((row, i) => [
//             (index * 10 + i + 1).toString(), // Correct numbering across pages
...
function generatePDF(
  ...
  if (logoBase64) {
    try {
      const logoWidth = pageWidth - margin * 2;
      const img = new Image();
      img.crossOrigin = "Anonymous"; // Unlikely needed for local assets
      img.src = logoBase64;
      img.onload = () => {
        const imgHeight = (img.height / img.width) * logoWidth;
        const maxHeight = 80;
        const scaleFactor = Math.min(1, maxHeight / imgHeight);
        const finalHeight = imgHeight * scaleFactor;
        doc.addImage(logoBase64, "JPG", margin, margin, logoWidth, finalHeight); // Use JPG for .jpg file
        yPos += finalHeight + 10;
      };
      img.onerror = () => {
        console.warn("Logo failed to load, skipping. Check base64 or image type.");
        yPos += 10;
      };
    } catch (e) {
      console.error("Error processing logo:", e);
    ...
    const convertImageToBase64 = () => {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          const base64 = canvas.toDataURL("image/jpeg"); // Use jpeg for .jpg
