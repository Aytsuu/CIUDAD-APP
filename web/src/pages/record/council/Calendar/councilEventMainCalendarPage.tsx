...
//   // Fetch mediation schedules
//   const { data: mediationSchedules = [], isLoading: isMediationLoading } = useGetMediationSchedules();
...
//   const { data: archivedEventsData, isLoading: isArchivedEventsLoading } = useGetCouncilEvents( 1, 1000, undefined, "all", true,  );
...
//     : (isArchivedEventsLoading || isArchivedWasteEventsLoading || isArchivedGADLoading || isMediationLoading);
...
function CalendarPage() {
  ...
  const isWaste = user?.staff?.pos?.toLowerCase() === "waste head" || user?.staff?.pos?.toLowerCase() === "admin";
