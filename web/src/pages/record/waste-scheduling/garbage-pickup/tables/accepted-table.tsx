...
export default function AcceptedTable() {
  ...
  const filteredRequests = selectedSitio === "0" 
    ? acceptedReqData.results 
    : acceptedReqData.results.filter(item => item.sitio_name === selectedSitio)
  ...
  const sitioOptions = [
    { id: "0", name: "All Sitios" },
    ...Array.from(new Set(acceptedReqData.results?.map(item => item.sitio_name) || []))
      .filter(name => name)
      .map(name => ({ id: name, name }))
  ]
      ...
      <div className="flex flex-col gap-4 p-6">
        ...
        <div className="flex items-center space-x-2">
          <h2 className="text-lg font-medium text-gray-800">
            Accepted Requests ({selectedSitio === "0" ? totalItems : totalDisplayedItems})
          </h2>
        </div>
