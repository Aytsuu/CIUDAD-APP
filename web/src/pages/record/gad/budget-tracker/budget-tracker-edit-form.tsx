...
//   const [_recordedBudgetItems, setRecordedBudgetItems] = useState<{ name: string; pax: string; amount: number }[]>([]);
//   const [activeVideoId, setActiveVideoId] = useState<string>("");
...
//         gbud_inc_amt: budgetEntry.gbud_inc_amt ? Number(budgetEntry.gbud_inc_amt) : 0,
//         gbud_exp_project: budgetEntry.gbud_exp_project || "",
//         gbud_exp_particulars: budgetEntry.gbud_exp_particulars || [],
//         gbud_actual_expense: budgetEntry.gbud_actual_expense ? Number(budgetEntry.gbud_actual_expense) : 0,
//         gbud_reference_num: budgetEntry.gbud_reference_num || "",
//         gbud_remaining_bal: budgetEntry.gbud_remaining_bal ? Number(budgetEntry.gbud_remaining_bal) : 0,
//         gbudy: yearBudgets?.find((b) => b.gbudy_year === year)?.gbudy_num || 0,
//         gpr_id: budgetEntry.gpr?.gpr_id || 0,
//       };
...
//           id: `receipt-${file.gbf_id}`,
//           type: "image" as const,
//           file: new File([], file.gbf_name, { type: file.gbf_type || "image/jpeg" }),
//           publicUrl: file.gbf_url,
//           storagePath: file.gbf_path,
//           status: "uploaded" as const,
//           previewUrl: file.gbf_url,
...
//       if (budgetEntry.gbud_type === "Expense" && budgetEntry.gbud_exp_particulars) {
...
//   if (!budgetEntry) return <div className="text-center py-8">No budget entry found.</div>;

//   const currentYearBudget = yearBudgets?.find((b) => b.gbudy_year === year);
//   const selectedProject = projectProposals?.find((p) => p.gpr_title === projectWatch);

//   return (
//     <div className="flex flex-col min-h-0 h-auto p-4 md:p-5 rounded-lg overflow-auto">
//       <div className="grid gap-4">
//         <Form {...form}>
//           <form onSubmit={form.handleSubmit(handleConfirmSave)} className="flex flex-col gap-4">
//             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
//               <div>
//                 <FormSelect
//                   control={form.control}
...
//                     <div className="p-2 border rounded">
//                       {budgetEntry.gbud_exp_project || "N/A"}
//                     </div>
//                   </div>
//                 )}
//               </div>
...
//                         <span className="text-sm text-gray-500 ml-2">{item.pax}</span>
//                       </div>
...
//               <ConfirmationModal
//                 trigger={
//                   <Button type="button" className="hover:bg-white-100 hover:opacity-[95%]">
//                     Save Changes
//                   </Button>
//                 }
//                 title="Confirm Save"
//                 description="Are you sure the details are accurate for this budget entry?"
//                 actionLabel="Confirm"
//                 onClick={() => form.handleSubmit(handleConfirmSave)()}
//               />
//             </div>
              ...
              {typeWatch === "Income" ? (
                <div className="flex flex-col gap-2">
                  ...
                  {!isEditing ? (
                    <div className="p-2 border rounded">
                      {budgetEntry.gbud_inc_amt
                        ? `₱${Number(budgetEntry.gbud_inc_amt).toLocaleString()}`
                        : "N/A"}
                    </div>
                  ) : (
                    <FormInput
                ...
                <>
                  ...
                  {isEditing ? (
                    ...
                    <div className="space-y-2">
                      <label className="block text-sm font-medium">Supporting Doc(s)</label>
                      {mediaFiles.map((file) => (
                        <div key={file.id} className="border rounded-md p-2">
                          <img
                            src={file.publicUrl}
