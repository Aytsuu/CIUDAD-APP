...
import { useUpdateProfileHealth } from "../../../health-family-profiling/family-profling/queries/profilingUpdateQueries";

export default function ResidentViewForm({ params }: { params: any }) {
  // ============= STATE INITIALIZATION ===============
  const { showLoading, hideLoading } = useLoading();
  const { mutateAsync: updateProfile } = useUpdateProfile();
  const { mutateAsync: updateProfileHealth } = useUpdateProfileHealth();
  ...
  // ================= SIDE EFFECTS ==================
  React.useEffect(() => {
    if (isLoadingFam || isLoadingPersonalInfo || isLoadingBusinesses)
      showLoading();
    else hideLoading();
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (
        checkDefaultValues(
          { ...values, per_addresses: addresses },
          {
            ...
            per_addresses: personalInfo?.per_addresses,
          }
        )
      ) {
        setIsSubmitting(false);
          ...
          ),
          {
            onSuccess: (new_addresses) => {
              const per_addresses = new_addresses.map((address: any) => ({
                add: address.add_id,
                per: params.data.personalInfo?.per_id || personalInfo?.per_id,
              }));
      ...
      // Use fallback for personalId
      const personalId =
        params.data.personalInfo?.per_id || personalInfo?.per_id;
      if (!personalId) {
        setIsSubmitting(false);
        handleSubmitError("No valid personal ID found for update.");
        return;
      }
      ...
      updateProfile(
        {
          ...
          values: {
            ...
            per_addresses: isAddressAdded
              ? addresses.slice(0, personalInfo?.per_addresses?.length)
              : addresses,
          },
        },
        {
          onSuccess: () => {
            updateProfileHealth({
              ...
              .then(() => {
                handleSubmitSuccess(
                  "Profile updated successfully in both databases"
                );
                setIsSubmitting(false);
                setFormType(Type.Viewing);
              })
              .catch((error) => {
                console.error("Health database update failed:", error);
                handleSubmitSuccess(
                  "Profile updated in main database (health database update failed)"
                );
                setIsSubmitting(false);
                setFormType(Type.Viewing);
              });
      ...
      <div className="grid gap-8">
        <Card className="w-full p-10">
          ...
          {isLoadingPersonalInfo ? (
            <ActivityIndicator message="Loading business information..." />
          ) : (
            <Form {...form}>
              <form
                onSubmit={(e) => {
        ...
        <Card className="w-full p-10">
          <div className="pb-4">
            ...
            <p className="text-xs text-black/50">
              Shows family details of this resident
            </p>
          </div>
          {renderFamilyContent()}
        </Card>
