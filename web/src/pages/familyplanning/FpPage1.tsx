...
//     setLoading(true)
//     try {
//         // const [patientsRes, spousesRes, measurementsRes, obsHistoryRes] = await Promise.all([
...
//       // setObstetricalHistory(obsHistoryData)
...
//       // console.log("✅ Obstetrical History fetched:", obsHistoryData)
//     } catch (error) {
...
//     )
//       const obsPatient = obstetricalHistory.find(
//         (obs) => obs.patrec_id === id)
...
//         // gravida: obsPatient?.gravida || 0,
//         // living_children: obsPatient?.living_children || 0,
//         // abortion: obsPatient?.abortion || 0,
//         // para: obsPatient.para || 0,
//         // fullterm: obsPatient?.fullterm || 0,
//         // premature: obsPatient?.premature || 0,

//       }
//        console.log("Patient",patientData.lastName)
...
// }
//   if(selectedPatient){
//     form.setValue("acknowledgement.clientName", `${selectedPatient.personal_info?.per_lname || ""}, ${selectedPatient.personal_info?.per_fname || ""} ${selectedPatient.personal_info?.per_mname || ""}`.trim())
//   }
...
//       if (!isChangingMethod) {
//         form.setValue("reason", "")
//         form.setValue("methodCurrentlyUsed","") // Clear methodCurrentlyUsed if not changing method
//         form.setValue("otherMethod", "")
...
export default function FamilyPlanningForm({ onNext2, updateFormData, formData, mode = "create" }: Page1Props) {
  ...
  const form = useForm<FormData>({
    ...
    values: getDefaultValues(formData), // Ensure values are also properly initialized
    mode: "onBlur",
  })
  ...
  const handlePatientSelection = (id: string) => {
    ...
    if (selectedPatient && selectedPatient.personal_info) {
      ...
      const patientMeasurements = bodyMeasurements.find((measurement) => measurement.pat === id)
      const obsPatient = obstetricalHistory.find((obs) => obs.patrec_id === id)
      ...
      const updatedFormData = {
        ...getDefaultValues(formData), // Use default values function
        ...patientData,
      }

      form.reset(updatedFormData)
      updateFormData(updatedFormData)
      console.log("✅ Auto-filled form with patient data:", updatedFormData)
    }
  }
