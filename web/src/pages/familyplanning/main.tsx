...
export default function FamilyPlanningPage() {
  ...
  const { id } = useParams<{ id: string }>(); // For potential view/edit mode
  ...
  // Use the master submission hook
  const { submitForm, isLoading, isError, error } = useFamilyPlanningFormSubmission();
  ...
  // useEffect(() => {
  //   if (id && (currentMode === "view" || currentMode === "edit")) {
  //     // TODO: Implement fetching complete record for edit/view mode using useQuery
  //     // For now, we'll keep the focus on creation.
  //     // You'd use a useQuery here, and update formData with the fetched data.
  //     // Example:
  //     // const { data: fetchedRecord, isLoading: isFetchingRecord } = useQuery({
  //     //   queryKey: ['fpCompleteRecord', id],
  //     //   queryFn: () => getFPCompleteRecord(Number(id)), // Your existing GetRequest function
  //     //   onSuccess: (data) => {
  //     //     setFormData(data); // Assuming fetched data matches FormData structure
  //     //   },
  //     //   enabled: !!id, // Only run if ID exists
  //     // });
  //     // If (isFetchingRecord) return <div>Loading record...</div>
  //   }
  // }, [id, currentMode]); // Add dependencies
  ...
  //       cough: apiData.medical_history?.coughMoreThan14Days || false,
 ...
 const handleSubmit = async () => {
    // This will now trigger the master submission hook
    try {
      // Ensure pat_id is present before submitting
      if (!formData.pat_id) {
        toast.error("Please select a patient on Page 1 before submitting.");
        return;
      }
      // Pass the entire formData to the master submitForm function
      await submitForm(formData);
      // On success, navigate to the overall records page
      navigate("/fam-planning-overall-table"); // Adjust this route as per your app's routing
    } catch (err) {
      // Errors are already handled by the hook's onError and toasts
      console.error("Submission initiated, but failed:", err);
    }
  };

    if (isLoading) {
    // You could replace this with a global loading spinner or a modal
    return (
      <div className="flex justify-center items-center h-screen text-lg font-semibold">
        Submitting Family Planning Record...
      </div>
    );
  }

  if (isError) {
    // Error messages are also displayed via toast from the hook
    return (
      ...
      <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg p-6 sm:p-8">
        ...
        {currentPage === 6 && (
          <FamilyPlanningForm6
            ...
            onSubmitFinal={handleSubmit} // This now calls the master handleSubmit
            updateFormData={updateFormData}
            formData={formData}
            isSubmitting={isLoading} // Pass isLoading from the hook
            mode={currentMode}
          />
        )}
      </div>
