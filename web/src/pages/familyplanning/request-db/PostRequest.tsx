...
export const createBodyMeasurement = async (data: Record<string, any>, patrec_id_for_bm: string) => {
  try {
    // Calculate BMI
    ...
    let bmi = 0;
     ...
     let patientRecordForBmId: number | null = null;
    try {
      const patientRecordRes = await api2.get(`familyplanning/patient-record-by-patient/${patrec_id_for_bm}/`); // Assuming this new endpoint exists
      patientRecordForBmId = patientRecordRes.data.patrec_id;
    } catch (error) {
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        // If no PatientRecord exists for this patient, create a generic one for BodyMeasurement
        console.log(`No existing PatientRecord for patient ${patrec_id_for_bm}, creating one for BodyMeasurement.`);
        const newPatientRecordRes = await api2.post("familyplanning/patient-record/", {
          patrec_type: "General Health", // Or "Body Measurement" or another appropriate type
          pat_id: patrec_id_for_bm,
        });
        patientRecordForBmId = newPatientRecordRes.data.patrec_id;
      } else {
        throw error; // Re-throw other errors
      }
    }

    if (!patientRecordForBmId) {
        throw new Error("Could not determine PatientRecord ID for Body Measurement.");
    }
...
export const updateBodyMeasurement = async (bm_id: number, data: Record<string, any>, patrec_id: number) => {
  try {
...
export const useCreateFPOBstetricalHistoryMutation = () => {
  return useMutation({
    mutationFn: ({
      ...
      fprecord_id: number;
      obs_id: number;
    }) => createFPOBstetricalHistory(data, fprecord_id, obs_id),
    onSuccess: () => {
...
export const useCreateBodyMeasurementMutation = () => {
  return useMutation({
    mutationFn: ({ data, patrec_id_for_bm }: { data: Record<string, any>; patrec_id_for_bm: string }) => // Updated arg
      createBodyMeasurement(data, patrec_id_for_bm), // Pass patrec_id
    onSuccess: () => {
...
export const useUpdateBodyMeasurementMutation = () => {
  return useMutation({
    mutationFn: ({ bm_id, data, patrec_id }: { bm_id: number; data: Record<string, any>; patrec_id: number }) =>
      updateBodyMeasurement(bm_id, data, patrec_id),
    onSuccess: () => {
...
export const useFamilyPlanningFormSubmission = (): UseFamilyPlanningFormSubmissionResult => {
  ...
  const submitForm = async (formData: FormData) => {
    try {
      ...
      const patrec_id = fpRecordResult.patrec_id; // This is the PatientRecord ID for FP (reused for BM and Assessment)
      ...
      let bm_id: number;
      // We will use the main patrec_id (from FP_Record) for BodyMeasurement
      const patrec_id_for_bm = patrec_id; 
      ...
      if (latestBm && latestBm.bm_id) {
        ...
        // Check if weight/height changed.
        ...
        if (
          ...
          console.log(`Updating existing body measurement (bm_id: ${bm_id})`);
          // Pass the existing patrec_id (from the fetched BM) for the update
          // This ensures we're updating the BM that's linked to its original patrec.
          const updatedBm = await updateBodyMeasurementMut.mutateAsync({
            bm_id: bm_id,
            data: formData,
            patrec_id: latestBm.patrec // Use the patrec_id from the *fetched* existing BM
          });
          bm_id = updatedBm.bm_id;
        } else {
          console.log(`Using existing body measurement (bm_id: ${bm_id}) as no changes detected.`);
        }
      } else {
        ...
        const newBm = await createBodyMeasurementMut.mutateAsync({
          data: formData,
          patrec_id_for_bm: patrec_id_for_bm, // Pass the patrec_id from main FP record
        });
        bm_id = newBm;
      }
      ...
      // --- Step 11: Assessment ---
      // The patrec_id passed here should be the main patrec_id from the FP_Record
