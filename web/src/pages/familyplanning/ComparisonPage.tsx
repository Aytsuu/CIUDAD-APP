...
type FullFPRecordDetail = {
  ...
  serviceProvisionRecords: { // This should be an array
    dateOfVisit: string;
    methodAccepted: string;
    methodQuantity: string | string[]; // methodQuantity is an array of strings in JSON
  ...
  medicalHistory: {
    ...
    breastCancerHistory: boolean; // Corrected type, assuming boolean from JSON
    severeChestPain: boolean; // Corrected type, assuming boolean from JSON
    cough: boolean;
    jaundice: boolean;
    unexplainedVaginalBleeding: boolean; // Corrected type, assuming boolean from JSON
    abnormalVaginalDischarge: boolean; // Corrected type, assuming boolean from JSON
        ...
        <div className="overflow-x-auto scroll-smooth mb-6">
          <div className="min-w-max bg-white rounded-lg shadow-md">
            ...
            >
              ...
              />
              <FieldRow
                label="Stroke/Heart Attack/Hypertension"
              ...
              />
              <FieldRow
                label="Jaundice (yellowish skin/eyes)"
            ...
            >
              ...
              <FieldRow
                ...
                values={recordsToDisplay.map(r => r.obstetricalHistory.livingChildren)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.obstetricalHistory.livingChildren))}
              />
            </Section>
            ...
            >
              <FieldRow
                label="Date of Visit"
                values={recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.dateOfVisit)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.dateOfVisit))}
              />
              <FieldRow
                label="Method Accepted"
                values={recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.methodAccepted)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.methodAccepted))}
              />
              <FieldRow
                label="Quantity of Method"
                values={recordsToDisplay.map(r =>
                  Array.isArray(r.serviceProvisionRecords[0]?.methodQuantity)
                    ? r.serviceProvisionRecords[0]?.methodQuantity.join(', ')
                    : r.serviceProvisionRecords[0]?.methodQuantity

                )}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.methodQuantity))}
              />
              <FieldRow
                label="Service Provider Name"
                values={recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.nameOfServiceProvider)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.nameOfServiceProvider))}
              />
              <FieldRow
                label="Date of Follow Up"
                values={recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.dateOfFollowUp)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.dateOfFollowUp))}
              />
              <FieldRow
                label="Service Provider Signature"
                values={recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.serviceProviderSignature)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.serviceProviderSignature))}
                isSignature
              />
              <FieldRow
                label="Medical Findings"
                values={recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.medicalFindings)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.serviceProvisionRecords[0]?.medicalFindings))}
              />
            </Section>
            ...
            >
              <FieldRow
                label="Breastfeeding"
                values={recordsToDisplay.map(r => r.fp_pregnancy_check.breastfeeding)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.fp_pregnancy_check.breastfeeding))}
                isBoolean
              />
              <FieldRow
                label="Abstained"
                values={recordsToDisplay.map(r => r.fp_pregnancy_check.abstained)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.fp_pregnancy_check.abstained))}
                isBoolean
              />
              <FieldRow
                label="Recent Baby"
                values={recordsToDisplay.map(r => r.fp_pregnancy_check.recent_baby)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.fp_pregnancy_check.recent_baby))}
                isBoolean
              />
              <FieldRow
                label="Recent Period"
                values={recordsToDisplay.map(r => r.fp_pregnancy_check.recent_period)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.fp_pregnancy_check.recent_period))}
                isBoolean
              />
              <FieldRow
                label="Recent Abortion"
                values={recordsToDisplay.map(r => r.fp_pregnancy_check.recent_abortion)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.fp_pregnancy_check.recent_abortion))}
                isBoolean
              />
              <FieldRow
                label="Using Contraceptive"
                values={recordsToDisplay.map(r => r.fp_pregnancy_check.using_contraceptive)}
                hasDifferences={hasDifferences(recordsToDisplay.map(r => r.fp_pregnancy_check.using_contraceptive))}
                isBoolean
              />
            </Section>
          </div>
