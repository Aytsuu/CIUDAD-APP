...
export default function WastedDoseForm({ wasted, record }: WastedDoseFormProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      alert(`Failed to record wasted dose: ${error.response?.data?.message || error.message}`);
    } finally {
  ...
  const handleVaccineWaste = async (record: StockRecords, wastedAmount: number) => {
    if (!isVaccine(record)) return;


    const  inventoryList = await api.get("inventory/vaccine_stocks/");

    const existingItem = inventoryList.data.find((item: any) => item.vacStck_id === record.vacStck_id);
    if (!existingItem) {
    ...
    await api.put(`inventory/vaccine_stocks/${record.vacStck_id}/`, updatePayload);
  ...
  const handleSupplyWaste = async (record: StockRecords, wastedAmount: number) => {
    ...
    if (record.imzStck_unit === "boxes") {
      ...
      if ('imzStck_pcs' in record) {
        updatePayload.imzStck_pcs = (Number(record.imzStck_pcs) || 0) - piecesToDeduct;
      }
    }

    await api.put(`inventory/immunization_stock/${record.imzStck_id}/`, updatePayload);
  ...
  const getFormLabel = () => {
    if (isVaccine(record)) {
      ...
      return `Wasted (enter ${record.imzStck_unit === "boxes" ? "boxes" : "pcs"})`;
    }
  };
  ...
  const getConfirmationMessage = (amount: number) => {
    if (isVaccine(record)) {
      ...
      if (record.imzStck_unit === "boxes") {
        const pcsPerBox = record.imzStck_per_pcs || 1;
        return `Are you sure you want to record ${amount} boxes (${amount * pcsPerBox} pcs)?`;
      }
  ...
  return (
    <>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      type="number"
                      placeholder={getFormLabel().replace("Wasted (", "Enter ").replace(")", "")}
                      value={field.value || ""}
                      onChange={(e) => {
      ...
      <ConfirmationDialog
        ...
        description={formData ? getConfirmationMessage(formData.wastedDose) : ""}
      />
    </>
  );
}