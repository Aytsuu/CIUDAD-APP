...
import { addVaccineStock,AntigenTransaction } from "../restful-api/VaccinePostAPI";
...
export const useSubmitVaccineStock = () => {
    ...
    return useMutation({
      mutationFn: async (data: Record<string,any>) => {
        const inventoryResponse = await addInventoryRecord({ data: data, inv_type: "Antigen" });
        ...
        if (!addedStock?.vacStck_id) throw new Error("Failed to get the new stock ID");
        ...
        await AntigenTransaction(addedStock.vacStck_id, string_qty, "Added", staffId);
  
        return { success: true };
      },
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["vaccineStockList"] }); // Adjust key if needed
      },
      onError: (error: any) => {
        console.error("Error submitting vaccine stock:", error.message || error);
      },
    });
  };