...
export const addFirstAidTransaction = async (
  FirstAidTransacindata: FirstAidTransactionType
) => {
  try {
    const res = await api.post(
      "inventory/firstaidtransaction/",
      FirstAidTransacindata
    );
    return res.data;
  } catch (err: any) {
    console.log("Error response:", err.response?.data || err.message);
...
export const FirstAidTransactionPayload = (
  finv_id: number,
  string_qty: string,
  action: string,
  staffId: number
...
export const FirstAidPayload = (
  ...
  // Replace with your actual return type if available
