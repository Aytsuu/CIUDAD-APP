...
import { CommodityType, CommodityListSchema } from "@/form-schema/inventory/lists/inventoryListSchema";
    ...
    <div>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <Label className="flex justify-center text-lg font-bold text-darkBlue2 text-center ">{mode === "edit" ? "Edit Commodity List" : "Add Commodity List"}</Label>
          <hr className="mb-2" />
          <FormInput control={form.control} name="com_name" label="Commodity Name" placeholder="Enter commodity name" />
          <FormSelect control={form.control} name="user_type" label="User type" options={user_type_options} />
          <FormSelect control={form.control} name="gender_type" label="For Gender" options={gender_type_options} />
