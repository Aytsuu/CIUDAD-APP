import { UseFormReturn } from "react-hook-form";
import { Form } from "react-router";
import { z } from "zod"

import { Button } from "@/components/ui/button";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";
import { Checkbox } from "@/components/ui/checkbox";
import { Separator } from "@/components/ui/separator";
import { CalendarCheck } from 'lucide-react';

import { PrenatalFormSchema } from "@/form-schema/maternal/prenatal-schema";
import { FormControl, FormField, FormItem, FormLabel } from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { useState } from "react";
import { Label } from "@/components/ui/label";


export default function PrenatalFormThirdPg(
    {form, onSubmit, back}: {
        form: UseFormReturn<z.infer<typeof PrenatalFormSchema>>,
        onSubmit: () => void,
        back: () => void,
    }
){
    const submit = () => {
        form.trigger(["followUpSchedule", "ancVisits", "checklist", "pregnancyPlan", "micronutrientSupp", "riskCodes", "assessedBy"]).then((isValid) => {
            if(isValid){
                onSubmit(); //proceed to next page
            }
        })
    }

    // for the schedule of follow-up visit 
    // -----
    const calculatedDate = (option: 'week' | 'twoweeks' | 'month'):string | null => {
        const today = new Date();

        switch (option){
            case 'week':
                today.setDate(today.getDate() + 7);
                break;
            case 'twoweeks':
                today.setDate(today.getDate() + 14);
                break;
            case 'month':
                today.setMonth(today.getMonth() + 1)
                break;
            default:
                return null;
        }
        return today.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric'});
    }

    const [selectedOption, setSelectedOption] = useState('');

    // validate for specific string
    const isValidOption = (value: string): value is 'week' | 'twoweeks' | 'month' => {
        return ['week', 'twoweeks', 'month'].includes(value);
    }


    // -----

    const items = [
        {
            id: "increasedBP", label: "INCREASED BP",
        },
        {
            id: "epigastricPain", label: "EPIGASTRIC PAIN",
        },
        {
            id: "nausea", label: "NAUSEA",
        },
        {
            id: "blurringOfVision", label: "BLURRING OF VISION",
        },
        {
            id: "edema", label: "EDEMA",
        },
        {
            id: "severeHeadache", label: "SEVERE HEADACHE",
        },
        {
            id: "abnormalVaginalDischarges", label: "ABNORMAL VAGINAL DISCHARGES",
        },
        {
            id: "vaginalBleeding", label: "VAGINAL BLEEDING",
        },
        {
            id: "chillsFever", label: "CHILLS & FEVER",
        },
        {
            id: "diffInBreathing", label: "DIFF. IN BREATHING",
        },
        {
            id: "varicosities", label: "VARICOSITIES",
        },
        {
            id: "abdominalPain", label: "ABDOMINAL PAIN",
        },
    ]


    return(
        <>
            <div className="flex flex-col min-h-0 h-auto md:p-10 rounded-lg overflow-auto">
                <Form {...form}>
                    <form onSubmit={(e) => {
                        e.preventDefault();
                        submit();
                    }}
                    >
                        <div className="grid grid-cols-2">
                            <div className="grid border rounded-lg m-2 border-gray">
                                <div className="flex flex-col m-5">
                                    <h2 className="text-xl font-bold">Schedule Follow-up Visit</h2>
                                    <FormField
                                        control={form.control}
                                        name="followUpSchedule.scheduleOption"
                                        render={({ field }) => (
                                            <FormItem className="ml-8 pt-3">
                                                <FormLabel className="text-semibold">Select follow-up period</FormLabel>
                                                <FormControl>
                                                    {/* radio button */}
                                                    <RadioGroup
                                                        onValueChange={(value) => {
                                                            field.onChange(value);
                                                            setSelectedOption(value);
                                                        }}
                                                        defaultValue={field.value}
                                                        className="flex flex-col space-y-1"
                                                    >
                                                        {/* after a week option */}
                                                        <FormItem className="flex items-center space-x-3 space-y-0"> 
                                                            <FormControl>
                                                                <RadioGroupItem value="week"/>
                                                            </FormControl>
                                                            <FormLabel>After a week (preferably for AOG within 9 months)</FormLabel>
                                                        </FormItem>

                                                        {/* after 2 week option */}
                                                        <FormItem className="flex items-center space-x-3 space-y-0"> 
                                                            <FormControl>
                                                                <RadioGroupItem value="twoweeks"/>
                                                            </FormControl>
                                                            <FormLabel>After 2 week (preferably for AOG within 7-8 months)</FormLabel>
                                                        </FormItem>

                                                        {/* after a week option */}
                                                        <FormItem className="flex items-center space-x-3 space-y-0"> 
                                                            <FormControl>
                                                                <RadioGroupItem value="month"/>
                                                            </FormControl>
                                                            <FormLabel>After a month (preferably for AOG within 1-6 months)</FormLabel>
                                                        </FormItem>
                                                    </RadioGroup>
                                                </FormControl>
                                            </FormItem>
                                        )}
                                    />
                                </div>
                                
                                {/* follow-up visit date display */}
                                <div className="flex items-center border rounded-lg ml-4 mr-4 mb-4 p-2 border-gray">
                                    <FormItem>
                                        <div className="flex">
                                            <CalendarCheck size={30} className="mr-2"/>
                                            <FormLabel className="flex items-center text-lg w-full">  Follow-up Date:</FormLabel>
                                        </div>
                                        <FormControl>
                                            <div className="flex justify-center w-full p-3 text-xl font-bold"> 
                                                {selectedOption && isValidOption(selectedOption) ? calculatedDate(selectedOption) : <i>Select a follow-up period</i>}
                                            </div>
                                        </FormControl>
                                    </FormItem>
                                </div>

                                <Button className="mr-4 mb-4 ml-4">Schedule Follow-Up</Button>
                            </div>
                            
                            {/* follow-up history */}
                            <div className="flex flex-col border rounded-lg m-2 border-gray h-[389px]">
                                <h2 className="text-xl font-bold m-5">Follow-up History</h2>
                                <div className="overflow-auto">
                                    <div className="flex flex-col border m-5 p-4 rounded-xl border-gray">
                                        <div className="w-full grid grid-cols-2 gap-2 mb-5">
                                            <div>
                                                <FormItem>
                                                    <FormLabel className="bg-gray rounded-2xl p-1 pr-2 pl-2">After a week</FormLabel>
                                                </FormItem>
                                            </div>
                                            
                                            <div className="flex justify-end">
                                                <FormItem>
                                                    <FormLabel className="text-ashgray p-1 pr-2 pl-2"><i>Created: March 04, 2025</i></FormLabel>
                                                </FormItem>
                                            </div>
                                        </div>
                                        <div className="flex items-center">
                                            <CalendarCheck size={30}/>
                                            <FormItem>
                                                <FormLabel className="text-2xl font-bold ml-3">March 11, 2025</FormLabel>
                                            </FormItem>
                                        </div>
                                    </div>


                                    <div className="flex flex-col border m-5 p-4 rounded-xl border-gray">
                                        <div className="w-full grid grid-cols-2 gap-2 mb-5">
                                            <div>
                                                <FormItem>
                                                    <FormLabel className="bg-gray rounded-2xl p-1 pr-2 pl-2">After a week</FormLabel>
                                                </FormItem>
                                            </div>
                                            
                                            <div className="flex justify-end">
                                                <FormItem>
                                                    <FormLabel className="text-ashgray p-1 pr-2 pl-2"><i>Created: February 25, 2025</i></FormLabel>
                                                </FormItem>
                                            </div>
                                        </div>
                                        <div className="flex items-center">
                                            <CalendarCheck size={30}/>
                                            <FormItem>
                                                <FormLabel className="text-2xl font-bold ml-3">March 04, 2025</FormLabel>
                                            </FormItem>
                                        </div>
                                    </div>
                                </div>
                                
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4 m-2">
                            {/* guide for 4ANC Visit */}
                            <div className="flex flex-col border w-full rounded-lg">
                                <h2 className="text-xl font-bold p-5">Guide for 4ANC Visit: (date)</h2>
                                <div>
                                    <FormField
                                        control={form.control}
                                        name="ancVisits.firstTri"
                                        render={({ field }) => (
                                            <FormItem className="mr-5 mb-5 ml-5">
                                                <FormLabel>1st tri up to 12 wks and 6 days</FormLabel>
                                                <FormControl>
                                                    <Input {...field} type="date"/>
                                                </FormControl>
                                            </FormItem>
                                        )}
                                    />
                                    <FormField
                                        control={form.control}
                                        name="ancVisits.secondTri"
                                        render={({ field }) => (
                                            <FormItem className="mr-5 mb-5 ml-5">
                                                <FormLabel>2nd tri 13-27 wks and 6 days</FormLabel>
                                                <FormControl>
                                                    <Input {...field} type="date"/>
                                                </FormControl>
                                            </FormItem>
                                        )}
                                    />
                                    <Label className="ml-5">3rd tri 28 wks and more</Label>
                                    <div className="grid grid-cols-2">
                                        <FormField
                                            control={form.control}
                                            name="ancVisits.thirdTriOne"
                                            render={({ field }) => (
                                                <FormItem className="mr-5 mb-5 ml-5">
                                                    <FormLabel>(1st visit in 3rd trimester)</FormLabel>
                                                    <FormControl>
                                                        <Input {...field} type="date"/>
                                                    </FormControl>
                                                </FormItem>
                                            )}
                                        />
                                        <FormField
                                            control={form.control}
                                            name="ancVisits.thirdTriTwo"
                                            render={({ field }) => (
                                                <FormItem className="mr-5 mb-5 ml-5">
                                                    <FormLabel>(2nd visit in 3rd Tri)</FormLabel>
                                                    <FormControl>
                                                        <Input {...field} type="date"/>
                                                    </FormControl>
                                                </FormItem>
                                            )}
                                        />
                                    </div>
                                    
                                </div>
                            </div>
                            {/* checklist */}
                            <div className="flex flex-col border w-full rounded-lg p-5">
                                <FormField
                                    control={form.control}
                                    name="checklist.0.increasedBP"
                                    render={({ field }) => (
                                        <FormItem className="flex flex-col">
                                            <FormLabel>CHECKLIST:</FormLabel>
                                            <FormLabel className="ml-5">( ) PRE-ECLAMPSIA</FormLabel>
                                            <div className="grid grid-cols-2">
                                                <div className="flex flex-col gap-4 mt-4 ml-12">
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                </div>
                                                <div className="flex flex-col gap-4 mt-4 ml-12">
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                    <Checkbox />
                                                </div>
                                            </div>
                                        </FormItem>
                                    )}
                                />
                            </div>
                        </div>

                        {/* pregnancy plan */}
                        <div className="grid grid-cols-2 gap-4">
                            <FormField
                                control={form.control}
                                name="pregnancyPlan.planPlacePfDel"
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>PLAN FOR PLACE OF DELIVERY</FormLabel>
                                        <FormControl>
                                            <Input {...field} placeholder="Enter plan of delivery place"/>
                                        </FormControl>
                                    </FormItem>
                                )}
                            />
                            <FormField
                                control={form.control}
                                name="pregnancyPlan.planNewbornScreening"
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>PLAN FOR NEWBORN SCREENING:</FormLabel>
                                        <FormControl>
                                            <RadioGroup
                                                onValueChange={(value) => {
                                                    field.onChange(value);
                                                    setSelectedOption(value);
                                                }}
                                            >
                                                <div className="flex ml-12">
                                                    <FormItem className="mr-4">
                                                        <FormControl>
                                                            <RadioGroupItem value="yes"/>
                                                        </FormControl>
                                                        <FormLabel className="ml-2">YES</FormLabel>
                                                    </FormItem>
                                                    <FormItem>
                                                        <FormControl>
                                                            <RadioGroupItem value="no"/>
                                                        </FormControl>
                                                        <FormLabel className="ml-2">NO</FormLabel>
                                                    </FormItem>
                                                </div>
                                                
                                            </RadioGroup>
                                        </FormControl>
                                    </FormItem>
                                )}
                            />
                        </div>

                        {/* risk codes */}
                        <div className="flex">

                        </div>

                        <div className="mt-8 sm:mt-auto flex justify-end">
                            <Button variant="outline" className="mt-4 mr-4 sm-w-32" onClick={back}>
                                Prev
                            </Button>
                            <Button type="submit" className="mt-4 mr-4 sm-w-32" onClick={onSubmit}>
                                Next
                            </Button>
                        </div>
                    </form>
                </Form>
            </div>
        </>
    )
}