...
export function AgeGroupForm({ isOpen, onClose, mode = "add", ageGroupData }: AgeGroupModalProps) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center">
            ...
            <Label className="text-xl text-darkBlue2">{isEditing ? "Edit Age Group" : "Add New Age Group"}</Label>
          </div>
          <Button variant="ghost" size="sm" onClick={handleCancel} className="h-8 w-8 p-0" aria-label="Close modal">
            <X className="h-4 w-4" />
          </Button>
        </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-4">
                <Button type="button" variant="outline" onClick={handleCancel} disabled={isLoading} aria-label="Cancel form">
                  Cancel
                </Button>
                <Button type="submit" disabled={isLoading} aria-label={isEditing ? "Update age group" : "Add age group"}>
                  {isLoading ? <span className="flex items-center">{isEditing ? "Updating..." : "Adding..."}</span> : isEditing ? "Update Age Group" : "Add Age Group"}
                </Button>
              </div>
            </form>
          </Form>
