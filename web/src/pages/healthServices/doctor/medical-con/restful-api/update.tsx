...
export const updateChildHistoryStatus = async (
  ...
  try {
    const response = await api2.patch(
      `child-health/update/history/${data.chhist_id}/`,data
    ...
    console.log(
      "Child health history status updated successfully:",
      response.data
    );
...
data: Record<string, any>,
  ...
  try {
    const response = await api2.patch(
      `child-health/update/child-vitalsigns/${data.chvital_id}/`,data    );
...
export const updateMedicalConsultation = async (data: Record<string, any>) => {
  try {
    const response = await api2.patch(
      `medical-consultation/update-medcon/${data.medrec_id}/`,
      data
    );
    if (!response.data) {
...
export const updatePEOption = async (pe_option_id: number, text: string) => {
  try {
    const res = await api2.patch(
      `patientrecords/update-pe-option/${pe_option_id}/`,
      { text }
    );
    if (res.data.error) {
