...
import {updateChildHistoryStatus,updateChildVitalSigns} from "../../restful-api/update";
...
export const useSubmitSoapForm = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const chvital_id = checkupData?.child_health_vital_signs?.[0]?.chvital_id || '';
      ...
      const chnotes_res =   await createChildHealthNotes({
        ...
        const vitalSignsData = {
            child_health_vital_signs: [{
              chvital_id: checkupData?.child_health_vital_signs?.[0]?.chvital_id || 0,
              // other fields...
            }]
          };
        ...
        await updateChildHistoryStatus(
            checkupData.chhist_id,
            "recorded"
        );

        // 6. Create Medical History if illnesses are selected
        if (formData.selectedIllnesses && formData.selectedIllnesses.length > 0) {
