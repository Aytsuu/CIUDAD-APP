...
import { getVaccineStock,deleteVaccinationHistory, createAntigenStockTransaction, createPatientRecord, createVaccinationRecord, createVaccinationHistory, createVitalSigns, createFollowUpVisit, deleteVaccinationRecord, deletePatientRecord, deleteVitalSigns, deleteFollowUpVisit } from '../restful-api/PostAPI';
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (assignmentOption === "other") {
        ...
        try {
          const vaccineData = await getVaccineStock(data.vaccinetype);
          ...
          if (vacrec_id) {
            await createVaccinationHistory(vacrec_id, data, data.vaccinetype, 0, "forwarded");
          } else {
    ...
    mutationFn: async ({
      ...
      try {
        ...
        const vaccinationRecord = await createVaccinationRecord(
          patrec_id, 
          status.toLowerCase() as "completed" | "forwarded" | "partially vaccinated", 
          maxDoses
        );
        vacrec_id = vaccinationRecord.vacrec_id;
