...
// import { createAntigenStockTransaction, createVaccinationHistory, createVitalSigns, createFollowUpVisit } from "../restful-api/post";
// import { deleteVaccinationRecord, deletePatientRecord, deleteVitalSigns, deleteFollowUpVisit } from "../restful-api/delete";
...
//         try {
//           const patientRecord = await createPatientRecord({ pat_id: data.pat_id, pat_type: "Vaccination Record", staff_id });
...
//         if (!vacStck_id) {
//           throw new Error("Vaccine ID is missing. Please select a valid vaccine type.");
//         }
...
//         const patientRecord = await createPatientRecord({ pat_id: pat_id ?? "", pat_type: "Vaccination Record", staff_id });
//         patrec_id = patientRecord.patrec_id;
...
//           const followUpVisit = await createFollowUpVisit(patrec_id, nextVisitDate.toISOString().split("T")[0], `Routine vaccination for ${vac_name} scheduled on ${nextVisitDate.toISOString().split("T")[0]}`, "pending");
//           followv_id = followUpVisit.followv_id;
//         } else if (vaccineData.vaccinelist.no_of_doses >= 2) {
//           const dose2Interval = vaccineData.vaccinelist.intervals.find((interval: { dose_number: number; interval: number; time_unit: string }) => interval.dose_number === 2);

//           if (dose2Interval) {
//             const nextVisitDate = calculateNextVisitDate(dose2Interval.interval, dose2Interval.time_unit, new Date().toISOString());
//             const followUpVisit = await createFollowUpVisit(patrec_id, nextVisitDate.toISOString().split("T")[0], `Follow-up visit for ${vac_name} scheduled on ${nextVisitDate.toISOString().split("T")[0]}`, "pending");
//             followv_id = followUpVisit.followv_id;
//           }
//         }
