...
export default function VaccinationView() {
  ...
  // Use the query hook
  const { data: vaccinationHistory = [], isLoading, error } = useIndivPatientVaccinationRecords(patientId);

  const currentVaccination = useMemo(() => {
    return vaccinationHistory.find((history: VaccinationRecord) => history.vachist_id === Vaccination?.vachist_id);
  ...
  return (
    <>
      <div className="flex items-center gap-4 mb-4 sm:mb-6">
        <Button className="text-darkGray p-2" variant="outline" onClick={() => navigate(-1)}>
          <ChevronLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="font-semibold text-xl sm:text-2xl text-darkBlue2">Vaccination Record</h1>
          <p className="text-xs sm:text-sm text-darkGray">View vaccination details and patient information</p>
        </div>
      </div>
