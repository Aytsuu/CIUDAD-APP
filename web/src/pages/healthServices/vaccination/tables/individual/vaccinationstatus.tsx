...
export function VaccinationStatusCards({
  ...
  const categorizedFollowups = {
    pending: followupVaccines.filter(vaccine => vaccine.followup_status === 'pending'),
    completed: followupVaccines.filter(vaccine => vaccine.followup_status === 'completed'),
    missed: followupVaccines.filter(vaccine => vaccine.followup_status === 'missed')
  };
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="bg-gray-100 rounded-xl p-6 flex-1 border border-gray-300 shadow-sm">
        ...
        <div className="flex justify-evenly gap-1 bg-gray-200 p-1 rounded-lg mb-6">
            ...
            >
            <Clock className={`h-4 w-4 ${activeTab === 'pending' ? 'text-blue-500' : 'text-gray-500'}`} />
            Pending
            ...
            >
            <CheckCircle className={`h-4 w-4 ${activeTab === 'completed' ? 'text-green-500' : 'text-gray-500'}`} />
            Completed
            <span className={`text-xs px-2 py-0.5 rounded-full ml-1 ${
              activeTab === 'completed' 
            ...
            >
            <AlertCircle className={`h-4 w-4 ${activeTab === 'missed' ? 'text-red-500' : 'text-gray-500'}`} />
            Missed
            <span className={`text-xs px-2 py-0.5 rounded-full ml-1 ${
              activeTab === 'missed' 
        ...
        {categorizedFollowups[activeTab].length > 0 ? (
          <ul className="space-y-3" role="list">
            {categorizedFollowups[activeTab].map((vaccine: any, index: number) => (
              <li
                key={index}
                className={`bg-white rounded-xl p-2 shadow-sm border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 ${
                  activeTab === 'pending' ? 'border-blue-200' : 
                  activeTab === 'completed' ? 'border-green-200' : 'border-red-200'
                }`}
                role="listitem"
              >
                <div className="flex items-center gap-3">
                  <div className={`w-4 h-4 rounded-full flex-shrink-0 shadow-sm ${
                    ...
                    activeTab === 'completed' ? 'bg-green-400' : 'bg-red-400'
                  }`}></div>
                  <span className="font-semibold text-gray-800">
                    {vaccine.vac_name}
                  </span>
                ...
                {vaccine.followup_date && (
                  <div className={`flex items-center gap-2 px-4 py-1 rounded-full ${
                    activeTab === 'pending' ? 'bg-blue-50 text-blue-800' : 
                    activeTab === 'completed' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
                  }`}>
                    <span className="text-sm font-semibold">
                      {activeTab === 'completed' ? 'Completed on: ' : 'Follow-up: '}
                      {new Date(vaccine.followup_date).toLocaleDateString("en-US", {
                        weekday: "short",
                        month: "short",
                        day: "numeric",
                        year: "numeric",
          ...
          <div className="text-center py-8" role="status">
            <div className="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center mx-auto mb-4 shadow-md">
              {activeTab === 'pending' && <Clock className="h-8 w-8 text-gray-700 " />}
              {activeTab === 'completed' && <CheckCircle className="h-8 w-8 text-gray-700" />}
              {activeTab === 'missed' && <AlertCircle className="h-8 w-8 text-gray-700" />}
            </div>
            <p className="text-gray-800 font-bold text-lg">
            ...
            <p className="text-sm text-gray-700  mt-1">
              {activeTab === 'pending' && 'No pending follow-up vaccines found for this patient.'}
              {activeTab === 'completed' && 'No completed follow-up vaccines found for this patient.'}
              {activeTab === 'missed' && 'No missed follow-up vaccines found for this patient.'}
            </p>
          </div>
        )}
      </div>
