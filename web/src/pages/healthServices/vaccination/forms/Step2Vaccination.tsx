    ...
    <div>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-sm shadow-sm border-gray-100">
        ...
        <CardLayout
          content={
            <>
              <h3 className="font-semibold text-lg text-blue-800 mb-4">Vaccination Details</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="">
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="font-medium text-gray-800">{params?.vaccineName || "-"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Vaccine Type</p>
                      <p className="font-medium text-gray-800">{params?.vaccineType || "-"}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Dose</p>
                      <p className="font-medium text-gray-800">{params?.vaccineDose ? `${params?.vaccineDose}${["st", "nd", "rd"][(params?.vaccineDose % 10) - 1] || "th"} Dose` : "-"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-sm text-gray-500">Total Doses Required</p>
                    <p className="font-medium text-gray-800">{params?.maxDoses || "-"}</p>
                  </div>

                  <div>
                    <p className="text-sm text-gray-500">Next Follow-up Visit</p>
                    <p className="font-medium text-gray-800">{params?.follow_up_visit?.followv_date ? format(new Date(params?.follow_up_visit.followv_date), "MMM dd, yyyy") : "No follow-up scheduled"}</p>
                  </div>
                  {/* <div>
        ...
        <Form {...form}>
          <form onSubmit={handleFormSubmit} className="space-y-6 mt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 bg-white">
              <FormInput control={form.control} name="pr" label="Pulse Rate (bpm)" placeholder="Enter pulse rate" type="number" />
              <FormInput control={form.control} name="temp" label="Temperature (°C)" placeholder="Enter temperature" type="number" />
              <FormInput control={form.control} name="o2" label="Oxygen Saturation (%)" placeholder="Enter SpO2 level" type="number" />
            </div>
            <h2 className="font-semibold text-blue  rounded-md py-2 px-3">Blood Pressure</h2>
            <div className="flex gap-2">
              <FormInput control={form.control} name="bpsystolic" label="Systolic Blood Pressure" type="number" placeholder="Systolic" />
              <FormInput control={form.control} name="bpdiastolic" label="Diastolic Blood Pressure" type="number" placeholder="Diastolic" />
            </div>

            <div className="flex justify-end gap-3 pt-6 pb-2">
              <Button type="button" variant="outline" className="w-[120px] border-gray-300 hover:bg-gray-50" onClick={() => navigate(-1)}>
                Cancel
              </Button>
