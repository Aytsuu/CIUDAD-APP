...
import { CommodityMonthItem } from "./types";
...
export default function InventoryMonthlyCommodityRecords() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useCommodityMonths(currentPage, pageSize, yearFilter, searchQuery);
  ...
  const columns: ColumnDef<CommodityMonthItem>[] = [
    {
      accessorKey: "month",
      header: "Month",
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: "Total Commodities",
      cell: ({ row }) => (
        <div className="text-center">{row.original.total_items.toLocaleString()}</div>
      ),
    },

    {
      ...
      cell: ({ row }) => (
        <Button
          onClick={() =>
            navigate("/inventory-monthly-commodity-details", {
              state: {
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-4">
        ...
        <div className="flex-col items-center ">
          <h1 className="font-semibold text-xl sm:text-2xl text-darkBlue2">
            Monthly Commodity Records
          </h1>
          <p className="text-xs sm:text-sm text-darkGray">
            View commodity transactions grouped by month ({totalMonths} months
            found)
          </p>
        </div>
      </div>
