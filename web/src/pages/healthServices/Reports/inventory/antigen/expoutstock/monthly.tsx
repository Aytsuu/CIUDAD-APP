// VaccinationExpiredOutOfStockSummary.tsx
...
export default function AntigenExpiredOutOfStockSummary() {
  ...
  const { data: apiResponse, isLoading, error } = useVaccinationExpiredOutOfStockSummary(currentPage, pageSize);

  useEffect(() => {
    if (error) {
      toast.error("Failed to fetch vaccination problem summary");
      toast("Retrying...");
      setTimeout(() => {
      ...
      <Card className="p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center mb-6">
          <div className="flex-1">
            <h2 className="text-lg font-semibold text-gray-900">Antigen Need Restock Summary</h2>
            <p className="text-sm text-gray-500">View expired and out-of-stock antigen items grouped by month</p>
          </div>
