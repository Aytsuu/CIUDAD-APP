...
export default function MonthlyInventoryAntigenRecords() {
  ...
  const { data: apiResponse, isLoading, error } = useVaccineMonths(currentPage, pageSize, yearFilter, searchQuery);

  useEffect(() => {
    if (error) {
      toast.error("Failed to fetch vaccine months");
      toast("Retrying...");
      setTimeout(() => {
  ...
  const monthlyData: VaccineMonthItem[] = apiResponse?.results?.data || [];
          ...
          <div className="bg-white w-full">
            {isLoading ? (
              <div className="w-full h-[300px] flex flex-col items-center justify-center text-gray-500">
                ...
                <span>Loading vaccine records...</span>
              </div>
              ...
              <div className="w-full h-[300px] flex flex-col items-center justify-center text-gray-500 p-8">
                ...
                <p className="text-sm text-center text-gray-400">{searchQuery ? "Try adjusting your search criteria" : "No vaccine records available"}</p>
              </div>
            )}
