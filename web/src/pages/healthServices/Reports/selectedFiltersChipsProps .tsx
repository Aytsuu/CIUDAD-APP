    ...
    return (
      <div className="bg-gray-50 p-3 flex flex-wrap gap-2">
        ...
        {items.map(item => (
          <span 
            key={item} 
            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${chipColor} ${textColor}`}
          >
            {getDisplayName(item)}
            <button
        ...
        <button
          onClick={onClearAll}
          className={`text-xs ${textColor} hover:opacity-80 ml-2`}
        >
          Clear all
        </button>
      </div>
