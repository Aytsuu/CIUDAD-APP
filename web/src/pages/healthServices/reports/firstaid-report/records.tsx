...
export default function MonthlyFirstAidDetails() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record: any) => {
      ...
      const address = patient?.address?.full_address?.toLowerCase() || "";
  ...
  const prepareExportData = () => {
    return filteredRecords.map((record: any) => {
      ...
      return {
        ...
        Name: fullName || "N/A",
        Address: patient?.address ? toTitleCase(`${patient.address.full_address}${patient.address.add_sitio ? " Sitio " + patient.address.add_sitio : ""}`) : "N/A",
  ...
  const tableRows = paginatedRecords.map((record: any) => {
    const patient = record.patrec_details;
    const personalInfo = patient?.personal_info;
    const fullName = toTitleCase([personalInfo?.per_fname, personalInfo?.per_mname, personalInfo?.per_lname].filter(Boolean).join(" "));

    const address = patient?.address ? toTitleCase(`${patient.address.full_address}${patient.address.add_sitio ? " Sitio " + patient.address.add_sitio : ""}`) : "N/A";

    return [
      record.created_at ? new Date(record.created_at).toLocaleDateString() : "N/A",
      <div className="items-center">{fullName || "N/A"}</div>,
      address,
            ...
            <div className="flex-1 max-w-md">
              ...
              {itemName && searchTerm === itemName && (
                <div className="text-xs text-blue-600 mt-1 flex items-center gap-1">
                  <span>
                    Showing results for: <strong>{itemName}</strong>
                  </span>
            ...
            <div className="p-4 print-area  " id="printable-area">
              ...
              <div className="content-area flex-1">
                ...
                <div className="text-center py-2 mb-6 pb-4">
                  ...
                  {searchTerm && <Label className="font-medium text-gray-700 block text-sm">Item: {searchTerm}</Label>}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 p-4">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-sm border-b border-black flex-1 pb-1">{currentSearchItem}</div>
                    </div>
                  </div>
              ...
              <div className="signature-section mt-auto pt-8">
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <div className="border-b border-black w-64 text-center pb-1 text-sm">{toTitleCase(position)}</div>
