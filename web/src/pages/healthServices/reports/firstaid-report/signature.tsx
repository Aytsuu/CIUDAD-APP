...
export const SignatureField = forwardRef<SignatureFieldRef, SignatureFieldProps>(({ title, onSignatureChange, initialSignature = "", required = false, className = "" }, ref) => {
  ...
  useEffect(() => {
    if (initialSignature) {
      const dataURL = initialSignature.startsWith("data:") ? initialSignature : `data:image/png;base64,${initialSignature}`;
        ...
        <div className="flex justify-end gap-2 mt-2">
          ...
          {!isEditing && signatureData && (
            <>
              ...
              <Button type="button" variant="ghost" size="sm" onClick={handleClear} className="text-destructive hover:text-destructive">
                Clear Signature
              </Button>
            </>
          )}
