...
export default function CommodityProblemDetails() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return allProblemItems.filter((item) => 
      ...
      item.received_from.toLowerCase().includes(lower)
    );
  ...
  const tableHeader = ["Commodity Name", "Expiry Date", "Received From", "Received", "Stocks Available", "Qty Used", "Status"];
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-4">
        ...
        <div className="flex-col items-center">
          <h1 className="font-semibold text-xl sm:text-2xl text-darkBlue2">Commodity Need Restocking Details - {monthName}</h1>
          <p className="text-xs sm:text-sm text-darkGray">
            Track commodities needing restocking - expired, out of stock, and near expiry -  ({summary.total_problems}  found)
          </p>
        </div>
      </div>
      ...
      <div id="printable-area" className="hidden">
        <div className="text-center mb-6">
          <h2 className="font-bold uppercase tracking-wide text-lg"> Commodity Restocking Summary
          - {monthName}</h2>
        </div>
        <table className="w-full border-collapse border border-gray-400">
          ...
          <tbody>
            {filteredRecords.map((item, index) => (
              <tr key={index}>
                <td className="border border-gray-400 p-2">{item.com_name}</td>
                <td className="border border-gray-400 p-2">{item.expiry_date}</td>
                <td className="border border-gray-400 p-2">{item.received_from}</td>
