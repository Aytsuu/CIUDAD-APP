// MonthlyFPRecords.tsx (new file in the same directory as "monthly.tsx")
...
export default function MonthlyFPRecords() {
  ...
  const [yearFilter] = useState<string>("all");  // Can make dynamic if needed
  ...
  return (
    <LayoutWithBack title="Monthly Family Planning Records" description={`View FP records grouped by month (${totalMonths} months found)`}>
      <div>
        <Card className="">
          ...
          <div className="h-full w-full rounded-md">
            ...
            <div className="w-full h-auto sm:h-16 bg-slate-50 flex flex-col sm:flex-row justify-between sm:items-center p-4 mb-4 gap-3">
              <div className="flex items-center gap-3">
                ...
                <Select value={pageSize.toString()} onValueChange={(value) => { setPageSize(Number.parseInt(value)); setCurrentPage(1); }}>
            ...
            <div className="bg-white w-full px-4 pt-3">
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                  {paginatedData.map((record: any) => (
                    <MonthInfoCard
                      ...
                      navigateTo={{
                        path: "/familyplanning/report/details",  // New details path
                        state: {
                ...
                <div className="w-full h-[300px] flex flex-col items-center justify-center text-gray-500 p-8">
                  ...
                  <p className="text-sm text-center text-gray-400">{searchQuery ? "Try adjusting your search criteria" : "No FP records available"}</p>
                </div>
              )}
              ...
              {paginatedData.length > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between w-full py-6 gap-3 border-t mt-6">
                  <p className="text-sm text-gray-600">
                    Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, filteredData.length)} of {filteredData.length} months
                  </p>
