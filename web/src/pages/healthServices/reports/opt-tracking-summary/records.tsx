...
export default function OPTMonthlyDetails() {
  ...
  const filteredSitios = sitios.filter((sitio: any) => sitio.sitio_name.toLowerCase().includes(debouncedSitioSearch.toLowerCase()));
      ...
      ageGroups.forEach((ageGroup) => {
        Object.keys(category.age_groups[ageGroup] || {}).forEach((status) => {
          if (status !== "Total") {
            ...
            if (totals[status]) {
              totals[status].Male += category.age_groups[ageGroup][status]?.Male || 0;
              totals[status].Female += category.age_groups[ageGroup][status]?.Female || 0;
            }
          }
        });
      });
