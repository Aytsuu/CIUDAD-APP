  ...
  export default function InvChildHealthRecords() {
    ...
    const filteredData = useMemo(() => {
      return historyData.filter((item: ChrRecords) => {
        const searchText = `${item.age} ${item.wt} ${item.ht} ${item.bmi} ${
          item.vaccineStat
        } ${item.nutritionStat} ${item.updatedAt} ${item.latestNote || ""}`.toLowerCase() // Use latestNote
    ...
    const columns: ColumnDef<ChrRecords>[] = [
      ...
      },
      {
        ...
        header: "Action",
        cell: ({ row }) => {
          const originalRecord = historyData.find((rec: any) => rec.chhist_id === row.original.chhist_id)
          const isLatest = originalRecord && new Date(originalRecord.rawCreatedAt).getTime() === latestHistoryCreatedAt
          return (
      ...
      <div className="w-full bg-snow">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex-col items-center mb-4">
            <h1 className="font-semibold text-xl sm:text-2xl text-darkBlue2">Individual Records</h1>
        ...
        <div className="w-full md:w-auto flex justify-end mb-2">
          <Button onClick={toChildHealthForm}>Add New Record</Button>
        </div>
          {" "}
            ...
            <div className="w-full h-auto sm:h-16 bg-white flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 sm:p-4 gap-3 sm:gap-0">
              <div className="flex gap-x-2 items-center">
                ...
                <Input
                  ...
                  className="w-14 h-8"
                  value={pageSize}
                  onChange={(e) => setPageSize(Math.max(1, Number.parseInt(e.target.value) || 10))}
                  min="1"
                />
