...
export default function ChildHealthRecordForm() {
  ...
  // Use localStorage for formData
  const [formData, setFormData] = useLocalStorage<FormData>("childHRFormData", initialFormData);
  ...
  const transformApiDataToFormData = (chhistRecord: any): FormData => {
    ...
    return {
      ...
      motherAge: familyHeadInfo?.family_heads?.mother?.personal_info?.per_dob ? calculateAge(familyHeadInfo.family_heads.mother.personal_info.per_dob).toString() : "",
      ...
      fatherAge: familyHeadInfo?.family_heads?.father?.personal_info?.per_dob ? calculateAge(familyHeadInfo.family_heads.father.personal_info.per_dob).toString() : "",
  ...
  useEffect(() => {
    const fetchRecordData = async () => {
      ...
      try {
        ...
        chrecRecord.child_health_histories?.forEach((history: any) => {
          ...
          const vitalSignsFromHistory: VitalSignType[] =
            history.child_health_vital_signs?.map((vital: any) => ({
              ...
              is_opt: vital.bm_details?.is_opt || false,
          ...
          if (history.immunization_tracking) {
            const extractedImmunization: ImmunizationTracking[] = history.immunization_tracking.map((track: any) => ({
              ...
              vaccine_name: track.vachist_details?.vaccine_stock?.vaccinelist?.vac_name || track.vachist_details?.vac_details?.vac_name || "Unknown",
              dose_number: track.vachist_details?.vachist_doseNo?.toString() || "",
              date_administered: track.vachist_details?.date_administered || track.vachist_details?.created_at?.split("T")[0] || "",
              status: track.vachist_details?.vachist_status || "completed",
              hasExistingVaccination: track.hasExistingVaccination || false,
              follow_up_date: track.vachist_details?.follow_up_visit?.followv_date || "",
              follow_up_status: track.vachist_details?.follow_up_visit?.followv_status || "pending",
              age_at_vaccination: track.vachist_details?.vachist_age || "",
              batch_number: track.vachist_details?.vaccine_stock?.batch_number || "",
              expiry_date: track.vachist_details?.vaccine_stock?.inv_details?.expiry_date || "",
            }));
          ...
          const nutritionalStatusFromHistory = history.nutrition_statuses?.[0]
            ? {
                ...
                muac: history.nutrition_statuses[0].muac ? Number.parseFloat(history.nutrition_statuses[0].muac) : undefined,
          ...
          const BFChecksFromHistory: BFCheck[] =
        ...
        const todaysVitalSign = allHistoricalVitalSigns.find((vital) => isToday(vital.date));
        if (todaysVitalSign) {
