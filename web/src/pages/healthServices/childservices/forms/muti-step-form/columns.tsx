...
import type { VitalSignType, NutritionalStatusType } from "@/form-schema/chr-schema/chr-schema";
...
export const createHistoricalNutritionalStatusColumns = (): ColumnDef<NutritionalStatusType>[] => [
  {
    accessorKey: "date",
    header: "Date",
    cell: ({ row }) => (row.original.date ? new Date(row.original.date).toLocaleDateString() : "N/A")
  },
  {
    accessorKey: "wfa",
    header: "WFA",
    cell: ({ row }) => {
      ...
      const description = NUTRITIONAL_STATUS_DESCRIPTIONS.wfa[status as keyof typeof NUTRITIONAL_STATUS_DESCRIPTIONS.wfa] || "Unknown";
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const description = NUTRITIONAL_STATUS_DESCRIPTIONS.lhfa[status as keyof typeof NUTRITIONAL_STATUS_DESCRIPTIONS.lhfa] || "Unknown";
      return `${status} - ${description}`;
    }
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const description = NUTRITIONAL_STATUS_DESCRIPTIONS.wfh[status as keyof typeof NUTRITIONAL_STATUS_DESCRIPTIONS.wfh] || "Unknown";
      return `${status} - ${description}`;
    }
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const description = NUTRITIONAL_STATUS_DESCRIPTIONS.muac[status as keyof typeof NUTRITIONAL_STATUS_DESCRIPTIONS.muac] || "Unknown";
      return `${status} - ${description}`;
    }
  },
  {
        ...
        <div className="max-w-[200px] text-left">
          ...
          {row.original.followUpVisit && (
            <p className="mt-1 flex flex-col text-xs text-gray-500">
              ...
              <span className="font-medium text-green-800">Status: {row.original.followv_status}</span>
            </p>
          )}
        </div>
      </div>
...
export const createTodaysVitalSignsColumns = (
...
): ColumnDef<VitalSignType>[] => [
  {
    ...
    cell: ({ row }) => {
      ...
      if (isEditing) {
        return <span>{row.original.date || "N/A"}</span>;
      }
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const isEditing = editingRowIndex === row.index;
      if (isEditing) {
        return <FormInput control={editVitalSignFormControl} name="ht" label="" type="number" placeholder="Height" className="w-full" />;
      }
  ...
  // ADD THIS NEW COLUMN FOR REMARKS
  {
    ...
    cell: ({ row }) => {
      const isEditing = editingRowIndex === row.index;
      if (isEditing) {
        return <FormTextArea control={editVitalSignFormControl} name="remarks" label="Remarks" placeholder="Enter remarks" rows={2} className="min-w-[200px]" />;
      }
      ...
      if (isEditing) {
        return (
          <div className="min-w-[200px] space-y-2">
            <FormTextArea control={editVitalSignFormControl} name="notes" label="Notes" placeholder="Enter notes" rows={3} />
            <FormTextArea control={editVitalSignFormControl} name="follov_description" label="Follow-up reason" placeholder="Follow-up reason" className="w-full" rows={3} readOnly={!!row.original.followv_id} />
            <FormDateTimeInput control={editVitalSignFormControl} name="followUpVisit" label="Follow-up date" type="date" readOnly={!!row.original.followv_id} />
          </div>
        );
      }
      ...
      return (
        <div className="flex justify-center">
          <div className="max-w-[200px] text-left">
            ...
            {row.original.followUpVisit && (
              <p className="mt-1 flex flex-col text-xs text-gray-500">
                {row.original.follov_description && <span>Follow up reason {row.original.follov_description}</span>}
                <span>
                  Schedule on {row.original.followUpVisit} ({row.original.followv_status || "N/A"})
                </span>
              </p>
            )}
          </div>
...
export const createHistoricalSupplementStatusColumns = (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return row.original.date_given_iron ? new Date(row.original.date_given_iron).toLocaleDateString() : "N/A";
    }
  },
  {
    ...
    cell: ({ row }) => {
      const isEditingAnemia = editingAnemiaIndex === row.index && row.original.status_type === "anemic";
      const isEditingBirthWeight = editingBirthWeightIndex === row.index && row.original.status_type === "birthwt";
      if (isEditingAnemia || isEditingBirthWeight) {
        return <FormDateTimeInput control={supplementStatusEditFormControl} name="date_completed" label="" type="date" />;
      }
