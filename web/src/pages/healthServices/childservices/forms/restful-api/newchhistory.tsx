...
import { createFollowUpVisit, createBodyMeasurement, createChildHealthNotes, createChildVitalSign, createExclusiveBFCheck, createSupplementStatus, createChildHealthHistory, processMedicineRequest } from "./createAPI";
    ...
    if (submittedData.historicalSupplementStatuses?.length) {
      ...
      const updates = submittedData.historicalSupplementStatuses
        ...
        .map((status) => {
          ...
          console.log("Date completed comparison:", originalStatus?.date_completed, "vs", status.date_completed);
          console.log("Has changed:", originalStatus?.date_completed !== status.date_completed);
