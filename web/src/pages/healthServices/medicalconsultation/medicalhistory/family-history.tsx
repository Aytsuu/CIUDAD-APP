  ...
  if (isLoading) {
    return (
      <div className="p-4 text-center">
        ...
        <span>Loading family medical history...</span>
      </div>
    );
  }

  if (isError || !famhistData) {
    return (
      <div className="p-4 flex items-center gap-2 text-red-500">
        <AlertCircle className="h-5 w-5" />
        <span>Failed to load family medical history</span>
      </div>
    );
  }

  const { ph_illnesses, other_illnesses } = famhistData;
          ...
          <tbody>
            {ph_illnesses?.data?.map((illness: any) => (
              <tr key={illness.ill_id} className="border-b border-black">
                ...
                <td className="pb-1 text-center border-r border-black">{illness.has_family_history ? "✓ ": ""}</td>
                <td className="pb-1 text-center border-r border-black">{!illness.has_family_history ? "✓ ": ""}</td>
                <td className="pb-1 text-sm text-gray-700">{illness.has_family_history && illness.year ? `Diagnosed in ${illness.year}` : ""}</td>
              </tr>
            ))}
            ...
            <tr className="border-b border-black">
              ...
              <td className="pb-1 text-center border-r border-black">{!other_illnesses || other_illnesses === "None" ? "✓ ": ""}</td>
              <td className="pb-2 first-line:text-sm text-gray-700">{other_illnesses && other_illnesses !== "None" ? other_illnesses : ""}</td>
            </tr>
          </tbody>
        </table>
      </div>
