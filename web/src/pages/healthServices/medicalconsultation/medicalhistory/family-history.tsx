          ...
          <tbody>
            {ph_illnesses?.data?.map((illness: any) => (
              <tr key={illness.ill_id} className="border-b border-black">
                ...
                <td className="text-center border-r border-black">{illness.has_family_history ? <Check className="w-4 h-4 mx-auto text-black" /> : ""}</td>
                <td className="text-center border-r border-black">{!illness.has_family_history ? <Check className="w-4 h-4 mx-auto text-black" /> : ""}</td>
                <td className="text-sm text-gray-700">{illness.has_family_history && illness.year ? `Diagnosed in ${illness.year}` : ""}</td>
              </tr>
            ))}
