...
// Import the reusable health warnings
...
export default function AllMedicalForm() {
  ...
  // Initialize form before useEffect
    ...
    const subscription = form.watch((value, { name }) => {
      ...
      if (name === "height" || name === "weight") {
        if (value.height && value.weight) {
          ...
          // Calculate age for BMI category
          const dob = selectedPatientData?.personal_info?.per_dob
            ? new Date(selectedPatientData.personal_info.per_dob)
            : null;
          let age = 0;
          if (dob) {
  ...
  }, [form, selectedPatientData, updateVitalWarnings, updateBMIResult]);
  ...
  const handlePatientSelect = async (patient: any, patientId: string) => {
    ...
    try {
      ...
      const prevMeasurements = await previousBMI(trimmedPatientId);
      if (prevMeasurements) {
        // Set the height and weight fields with previous values
        form.setValue("height", prevMeasurements.height);
        form.setValue("weight", prevMeasurements.weight);
        console.log("height", prevMeasurements.height);
        console.log("weight", prevMeasurements.weight);
      }
    } catch (error) {
  ...
  const onSubmit = async (data: nonPhilHealthType) => {
    ...
    try {
      ...
      const bmiCategory = getBMICategory(bmiValue, age);
      console.log("Calculated BMI:", bmiValue, "Category:", bmiCategory);
      ...
      console.log(calculateAge(selectedPatientData?.personal_info?.per_dob));
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="bg-white rounded-lg shadow p-4 sm:p-8 mb-6 mt-5">
            ...
            {/* Use the reusable HealthWarnings component */}
            <HealthWarnings
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button
              type="submit"
              className="w-full sm:w-[150px]"
              disabled={!selectedPatientId || form.formState.isSubmitting}
            >
