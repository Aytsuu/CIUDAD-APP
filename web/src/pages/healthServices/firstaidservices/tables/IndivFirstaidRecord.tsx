...
import { ArrowUpDown, Search, AlertCircle, Heart } from "lucide-react";
...
export default function IndivFirstAidRecords() {
  ...
  const firstAidRecords = useMemo(() => {
    if (!firstAidRecordsResponse) return [];
  ...
  // Extract and serialize patient data from first aid records for display purposes only
  const derivedPatientData: any | null = useMemo(() => {
    if (!firstAidRecords || firstAidRecords.length === 0) {
      return null;
    }
    const firstRecord = firstAidRecords[0];
    const patientDetails =firstRecord?.patrec_details 
    if (!patientDetails) {
      return null;
    }
    try {
      return serializePatientData ? serializePatientData(patientDetails) : patientDetails;
    } catch {
  ...
  const formatFirstAidData = useCallback((): FirstAidRecord[] => {
    if (!Array.isArray(firstAidRecords)) return [];
