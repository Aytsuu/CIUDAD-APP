...
import { getFirstAidStocksList } from "@/pages/healthInventory/inventoryStocks/REQUEST/FirstAid/restful-api/FirstAidGetAPI";
import { updateFirstAidStock, updateInventoryTimestamp } from "@/pages/healthInventory/inventoryStocks/REQUEST/FirstAid/restful-api/FirstAidPutAPI";
...
export const processFirstRequest = async (data: any): Promise<any[]> => {
  ...
  for (const fa of data.firstaid) {
    try {
      ...
      const inventoryList = await getFirstAidStocksList();
      const existingFirstaid = inventoryList.find((item: any) => parseInt(item.finv_id, 10) === parseInt(fa.finv_id, 10));
