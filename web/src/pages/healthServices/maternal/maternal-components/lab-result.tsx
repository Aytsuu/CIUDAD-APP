...
const ImageUploadSection = ({ labName, labData, onImageUpload, onImageRemove }: { labName: string; labData: LabResultData; onImageUpload: (labName: string, files: File[]) => void; onImageRemove: (labName: string, index: number) => void }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${dragActive ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:border-gray-400"}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <label htmlFor={`file-input-${labName}`} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 cursor-pointer transition-colors">
      ...
      {/* Image Preview Modal */}
      {showPreview && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setShowPreview(null)}>
          <div className="relative max-w-4xl w-full max-h-[90vh] bg-white rounded-lg overflow-hidden">
            ...
            <img src={showPreview || "/placeholder.svg"} alt="Lab result preview" className="w-full h-auto max-h-[85vh] object-contain" onClick={(e) => e.stopPropagation()} />
          </div>
        </div>
      )}
    </div>
...
const LabResultItem = ({
  ...
  return (
    <div className={`space-y-4 border rounded-lg p-4 transition-all duration-200 ${labData.checked ? "border-blue-200 bg-blue-50/30" : "border-gray-200 bg-white"} ${error ? "border-red-300 bg-red-50/30" : ""}`}>
      <div className="flex items-start space-x-3">
        <div className="flex items-center space-x-2 flex-1">
          <Checkbox checked={labData.checked} onCheckedChange={(checked) => onLabCheckbox(labName, checked as boolean)} className="mt-1" />
      ...
      {isExpanded && (
        <div className="ml-6 space-y-4 animate-in slide-in-from-top-2 duration-200">
          <div className="flex items-center space-x-2">
            <Checkbox checked={labData.toBeFollowed} onCheckedChange={(checked) => onToBeFollowed(labName, checked as boolean)} />
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full mt-1 px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${!labData.date ? "border-red-300 bg-red-50/30" : "border-gray-300"}`}
                />
                {!labData.date && <p className="text-xs text-red-600 mt-1">Date is required when test is completed</p>}
              </div>

              {/* Image Upload Section */}
              <ImageUploadSection labName={labName} labData={labData} onImageUpload={onImageUpload} onImageRemove={onImageRemove} />
          ...
          {labData.toBeFollowed && (
            <div className="text-sm text-orange-600 bg-orange-50 p-3 rounded-md border border-orange-200">
              ...
              <p className="mt-1 text-orange-700">This laboratory test result will be uploaded after the patient has completed the procedure.</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="p-4 space-y-6">
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg border">
            ...
            <div className="space-y-2">
              {Object.entries(labResults)
                ...
                .map(([labName, lab]) => (
                  <div key={labName} className="flex justify-between items-center py-2 border-b border-gray-200 last:border-b-0">
                    <div className="flex-1">
                      <span className="font-medium text-sm">{LAB_DISPLAY_NAMES[labName as keyof typeof LAB_DISPLAY_NAMES].name}</span>
        ...
        {statistics.incomplete > 0 && (
          <Alert variant="destructive">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>{statistics.incomplete} test(s) are missing required dates. Please provide dates for all completed tests.</AlertDescription>
          </Alert>
        )}
      </div>
    </>
