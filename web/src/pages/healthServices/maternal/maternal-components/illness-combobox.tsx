...
export const IllnessCombobox = React.memo(
  ({
    options,
    value,
    onChange,
    ...
    onAddNew,
    allowAddNew = false,
    allowDeselect = false, // New prop to control deselection behavior
  }: {
    ...
    const triggerButton = customTrigger ? (
      <div role="combobox" aria-expanded={open} onClick={() => setOpen(true)}>
        {customTrigger}
      </div>
    ) : (
      <Button
        ...
        role="combobox"
        aria-expanded={open}
        className={cn("h-15 justify-between text-black/80 font-normal", triggerClassName)}
        onClick={() => setOpen(true)}
      >
        {!staticVal ? (
          <>
            <div className="truncate text-left flex-1 font-normal">
          ...
          <div className="truncate text-left flex-1 font-normal">{value}</div>
        )}
      </Button>
    )
                ...
                <div className="flex-1 min-w-0 font-normal">
                  {React.isValidElement(option.name)
                    ? React.cloneElement(option.name as React.ReactElement, {
                        className: cn(
                          "flex flex-wrap gap-2 items-center font-normal",
