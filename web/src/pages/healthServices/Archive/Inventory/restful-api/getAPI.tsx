...
export const getCommodityStocks = async () => {
  try {
    const res = await api2.get("inventory/archive/commodity-inventory/");
    if (res.status == 200) {
      return res.data;
    }
...
export const getMedicineInventory = async () => {
  try {
    const res = await api2.get(`inventory/archive/medicine-inventory/`);
    return res.data;
  } catch (err: any) {
    console.log(
      "Error fetching medicine inventory:",
...
export const getCombinedStock = async () => {
  try {
    const [vaccineStocks, supplyStocks, vaccines, supplies, inventory] =
      await Promise.all([
        api2.get("inventory/archive/vaccine-stocks/"),
        api2.get("inventory/archive/immunization-supplies/"),
