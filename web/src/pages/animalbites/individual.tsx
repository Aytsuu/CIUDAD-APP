...
//   try {
//     // Delete the bite details first
//     await api2.delete(`animalbites/details/${recordId}/`)
...
//           variant="destructive"
//           size="sm"
//           onClick={() => deleteRecord(row.original.bite_id)}
//           className="h-8 px-2 text-xs"
//         >
//           Delete
//         </Button>
...
type PatientRecord = {
  ...
  transient: boolean
  exposure: string
  siteOfExposure: string | { exposure_site: string }
  bitingAnimal: string | { animal_name: string }
  actions: string
  referredBy: string
  lab_exam: string
}

function IndividualAnimalBite() {
  const { id } = useParams<{ id: string }>()
  const [searchParams] = useSearchParams()
  const idType = searchParams.get("type") || "patient" // Default to patient if not specified
  const navigate = useNavigate()
  const queryClient = useQueryClient()
  const [isRefreshing, setIsRefreshing] = useState(false)
  ...
  const {
    ...
    ? usePatientRecordsByReferralId(id || "")
    : usePatientRecordsByPatId(id || "")
  ...
  const columns: ColumnDef<PatientRecord>[] = [
    ...
    },
    {
      ...
      header: "Site of Exposure",
      cell: ({ row }) => {
        const site = row.original.siteOfExposure
        if (typeof site === "object" && site !== null) {
          return site.exposure_site || "N/A"
        }
        return site || "N/A"
      },
    },
    {
      ...
      header: "Biting Animal",
      cell: ({ row }) => {
        const animal = row.original.bitingAnimal
        if (typeof animal === "object" && animal !== null) {
          return animal.animal_name || "N/A"
        }
        return animal || "N/A"
      },
    },
        ...
        return (
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" size="sm" onClick={() => setRecordToDelete(record.id)}>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete this animal bite record.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
  ...
  const handleDeleteRecord = async (referralId: number) => {
    setIsDeleting(true)
    try {
      await deleteAnimalBiteRecord(referralId)
      toast.success("Record deleted successfully")
      
      // Refresh the data
      await refetch()
      
      // If this was the last record, go back to the overall view
      if (records.length <= 1) {
        navigate("/animal-bites")
      }
      
      // Invalidate the overall patients query to refresh the list
      queryClient.invalidateQueries({ queryKey: ["unique-animalbite-patients"] })
    } catch (error) {
      console.error("Error deleting record:", error)
      toast.error("Failed to delete record")
    ...
    return (
      <div className="p-4">
        <div className="flex items-center mb-4">
          <Button variant="outline" onClick={() => navigate("/animal-bites")} className="mr-4">
    ...
    return (
      <div className="p-4">
        ...
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
          <p>Error loading patient data: {(error as Error).message}</p>
        </div>
    ...
    <div className="p-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">
            {patientInfo.per_fname} {patientInfo.per_mname} {patientInfo.per_lname}
          </h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-gray-50 p-4 rounded-lg">
        <div>
          ...
          <p className="font-medium">{patientInfo.pat_id || "N/A"}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Gender</p>
          <p className="font-medium">{patientInfo.per_sex || "N/A"}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Date of Birth</p>
          <p className="font-medium">{patientInfo.per_dob || "N/A"}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Address</p>
          <p className="font-medium">{patientInfo.per_address || "N/A"}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Contact</p>
          <p className="font-medium">{patientInfo.per_contact || "N/A"}</p>
        </div>
        <div>
      ...
      <h2 className="text-xl font-semibold mb-4">Animal Bite Records</h2>
      <DataTable columns={columns} data={records} />
    </div>
  )
}

export default IndividualAnimalBite