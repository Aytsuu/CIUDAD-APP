...
const currentDate = referenceDate ? new Date(referenceDate) : new Date();
  ...
  if (totalYears >= 1) {
    ...
    if (remainingMonths > 0) {
      ageString = `${totalYears} year${totalYears > 1 ? "s" : ""} and ${remainingMonths} month${remainingMonths > 1 ? "s" : ""}`
    } else {
...
export function getAgeInYearsOnly(dob: string | Date, referenceDate?: string | Date): number {
...
export function getAgeInYearsAndMonths(dob: string | Date, referenceDate?: string | Date): {
  years: number
  months: number
  formatted: string
} {
  ...
  const formatted =
    age.months > 0
      ? `${age.totalYears} year${age.totalYears > 1 ? "s" : ""} and ${age.months} month${age.months > 1 ? "s" : ""}`
