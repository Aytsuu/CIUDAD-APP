...
class ReportsCount(APIView):
    def get(self, request):
        try:
            ...
            medicalcon_count = MedicalConsultation_Record.objects.filter(status='completed').count()
            vaccnerecord_count =vaccination_completed_count = VaccinationHistory.objects.filter(status='completed').count()
