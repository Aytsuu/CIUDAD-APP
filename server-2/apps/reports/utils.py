...
# REUSABLE FUNCTIONS FOR BODY MEASUREMENT REPORTS
...
def apply_search_filter_to_body_measurement(queryset, search_query):
    ...
    for term in search_terms:
        ...
        # Search by child/patient name (both resident and transient) - case insensitive
        name_query |= (
            Q(pat__rp_id__per__per_fname__icontains=term_lower) |
            Q(pat__rp_id__per__per_mname__icontains=term_lower) |
            Q(pat__rp_id__per__per_lname__icontains=term_lower) |
            Q(pat__trans_id__tran_fname__icontains=term_lower) |
            Q(pat__trans_id__tran_mname__icontains=term_lower) |
            Q(pat__trans_id__tran_lname__icontains=term_lower)
        )

        # Handle full name search (e.g., "Juan Dela Cruz")
        if ' ' in term:
            name_parts = term.split()
            if len(name_parts) >= 2:
                # Search for combinations of first name + last name
                first_name_part = name_parts[0].lower()
                last_name_part = name_parts[-1].lower()
                
                name_query |= (
                    # Resident: first name + last name
                    Q(pat__rp_id__per__per_fname__icontains=first_name_part) &
                    Q(pat__rp_id__per__per_lname__icontains=last_name_part)
                ) | (
                    # Transient: first name + last name
                    Q(pat__trans_id__tran_fname__icontains=first_name_part) &
                    Q(pat__trans_id__tran_lname__icontains=last_name_part)
                )
                
                # Also search for last name + first name combination
                name_query |= (
                    Q(pat__rp_id__per__per_lname__icontains=first_name_part) &
                    Q(pat__rp_id__per__per_fname__icontains=last_name_part)
                ) | (
                    Q(pat__trans_id__tran_lname__icontains=first_name_part) &
                    Q(pat__trans_id__tran_fname__icontains=last_name_part)
                )

        # Search by sitio for residents (case-insensitive and partial match)
        matching_person_ids = PersonalAddress.objects.filter(
            Q(add__add_external_sitio__icontains=term_lower) |
            Q(add__sitio__sitio_name__icontains=term_lower)
        ).values_list('per', flat=True)
        person_ids.update(matching_person_ids)

        # Search by sitio for transients (case-insensitive and partial match)
        matching_transient_ids = Transient.objects.filter(
            Q(tradd_id__tradd_sitio__icontains=term_lower)
        ).values_list('trans_id', flat=True)
        transient_ids.update(matching_transient_ids)
...
def apply_age_filter_to_body_measurement(queryset, age_range):
    """Apply age range filter to BodyMeasurement queryset"""
    try:
        ...
        for obj in queryset:
            ...
            # Calculate age in months
            age_months = calculate_age_in_months(dob, created_at)
