...
class RTEUpdateSerializer(serializers.ModelSerializer):
  ...
  logo_top = FileInputSerializer(write_only=True, required=False)
  ...
  class Meta:
    model = ReportTemplate
    fields = ['logo_right', 'logo_left', 'logo_top', 'rte_prepared_by', 'rte_recommended_by', 
              'rte_approved_by']
  ...
  def update(self, instance, validated_data):
    ...
    if logo_right:
      ...
      url = upload_to_storage(logo_top, 'report-bucket', 'template')
      if url:
        setattr(instance, 'rte_logoTop', url)
    else:
