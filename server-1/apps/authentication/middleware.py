...
class AuthCheckingMiddleware:
    ...
    def __call__(self, request):
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            ...
            return JsonResponse({'error': 'Unauthorized', 'message': 'Authorization token required'}, status=401)
        ...
        try:
            ...
            if not supabase_user:
                ...
                return JsonResponse({'error': 'Unauthorized', 'message': 'Token invalid or expired'}, status=401)
            ...
            return JsonResponse({'error': 'Unauthorized', 'message': 'Token validation failed'}, status=401)
